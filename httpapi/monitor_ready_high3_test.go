...
// TestHigh3_LegacyReadyBlindToRouteFault proves the legacy no-level /ready probe
...
func TestHigh3_LegacyReadyBlindToRouteFault(t *testing.T) {
	...
	assert.Equal(t, http.StatusServiceUnavailable, legacy.Code,
		"legacy /ready must return 503 when an isolated route is faulted (HIGH-3)")
...
// HIGH-3 legacy /ready gate (>= LevelFull) does NOT strand the canonical simple
...
// /ready 503 FOREVER for a perfectly healthy bridge — a regression the HIGH-3
