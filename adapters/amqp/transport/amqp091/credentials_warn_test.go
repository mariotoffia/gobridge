...
// managed credentials (the residual gap).
//
// injectCredentials already makes explicit/rotated credentials WIN over
// userinfo embedded in broker_url (the core fix). The residual gap: the
...
func TestApplyCredentials_WarnsWhenBrokerURLEmbedsUserinfo(t *testing.T) {
	...
	// The core behavior is unchanged: rotated credentials win.
