...
// --- FIX 1: SASL EXTERNAL cert-material check is deferred while a
...
// longer wrongly rejected at parse time (F10 URI path). ---

func TestConfig_ExternalSASL_DeferredCredentials_FIX1(t *testing.T) {
	...
	t.Run("deferred: ApplyCredentials without cert restores F10 teeth", func(t *testing.T) {
		c := &Config{
			Session:           SessionOptions{Address: "amqps://broker:5671", SASLMechanism: "external"},
			CredentialsURIRef: "vault://amqp/mtls",
		}
...
// --- FIX 2: application-property values are rendered to domain-safe
// stdlib types so no go-amqp SDK type crosses the ACL into envelope
// headers (F9 application-property path). ---

func TestMessageToHeaders_AppPropertiesRenderStdlib_FIX2(t *testing.T) {
	...
	require.False(t, hasReserved, "FIX 2: reserved headers must still be filtered")
	_, hasPrefixed := h[headerPrefix+"internal"]
	require.False(t, hasPrefixed, "FIX 2: amqp10.* headers must still be filtered")
}

// --- FIX 3: a context.Canceled settlement cause is a deliberate route
...
func TestReceiver_SettlementFailed_ContextCanceled_NoRebuild_FIX3(t *testing.T) {
	rec := &ports.RecordingExporter{}
	// LinkCredit 2 -> threshold 1: absent the FIX-3 guard the FIRST
	...
	require.Equal(t, 0, fl.closeCalls,
		"FIX 3: context.Canceled settlements are deliberate teardown, never a rebuild trigger")
	r.settleFailMu.Lock()
	got := r.settleFailures
	r.settleFailMu.Unlock()
	require.Equal(t, 0, got, "FIX 3: canceled settlements must not increment the failure counter")
	require.Empty(t, rec.FindEntries(MetricAMQP10SettleFailed),
		"FIX 3: a deliberate teardown is not a health signal and emits no failure metric")
}

// --- FIX 4: a stale in-flight settlement from a superseded link
// generation must not be counted against the freshly rebuilt link
// (which would trip a spurious second rebuild). ---

func TestReceiver_SettlementFailed_StaleGeneration_NoIncrement_FIX4(t *testing.T) {
	rec := &ports.RecordingExporter{}
	...
	require.Equal(t, 0, fl.closeCalls,
		"FIX 4: a stale-generation settlement must not trigger a rebuild on the new link")
	r.settleFailMu.Lock()
	got := r.settleFailures
	r.settleFailMu.Unlock()
	require.Equal(t, 0, got, "FIX 4: a stale-generation settlement must not increment the current counter")
	require.Len(t, rec.FindEntries(MetricAMQP10SettleFailed), 1,
		"FIX 4: a stale settle failure stays observable via the metric")
	...
	require.Equal(t, 1, fl.closeCalls,
		"FIX 4: a current-generation settlement failure still forces the rebuild")
}

// --- FIX 5: the group-sequence header accepts the numeric carriers a
// header can arrive as (notably float64 from JSON), bounded to a valid
// uint32, instead of silently dropping non-int values (F12). ---

func TestHeadersToMessage_GroupSequence_BroadNumeric_FIX5(t *testing.T) {
	...
	if frac.Properties != nil && frac.Properties.GroupSequence != nil {
		t.Fatalf("FIX 5: a fractional group-sequence must be dropped, got %d", *frac.Properties.GroupSequence)
	}
	...
	if over.Properties != nil && over.Properties.GroupSequence != nil {
		t.Fatalf("FIX 5: an out-of-range group-sequence must be dropped, got %d", *over.Properties.GroupSequence)
	}
}
