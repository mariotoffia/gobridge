...
// audit fixes (F1..F14). No time.Sleep: races are driven with
...
// --- F1: tls.enable on a non-TLS scheme is a cleartext downgrade trap ---
...
// --- F5: a plan with fewer active receivers than wanted is Degraded ---
...
func TestSession_Health_UnderProvisioned_Degraded_F5(t *testing.T) {
	...
	// routes from readiness — F5 requires Degraded.
	s := newTestSession()
	s.mu.Lock()
	s.conn = &mockConn{}
	s.connected = true
	...
	require.Equal(t, ports.ServiceLevelDegraded, h.ServiceLevel,
		"F5: connected with active < wanted must be Degraded, not Full")
}

// --- F7: an inbound ttl-only message gets a concrete egress expiry ---
...
// --- F8: a Done-driven connection loss marks senders down (symmetric) ---
...
func TestSession_HandleConnLost_MarksSendersDown_F8(t *testing.T) {
	...
	// that would re-establish and clear the down state), so the F8
	// down-marking is observable deterministically without racing.
	...
	up := s.senders[snd]
	s.mu.Unlock()
	require.False(t, up, "F8: sender must be marked down after a Done-driven connection loss")
}

// --- F9: typed / binary message-ids render to clean strings in headers ---
...
// --- F11: max_frame_size below the AMQP 1.0 minimum (512) is rejected ---
...
// --- F12: group-sequence above MaxUint32 is dropped, not wrapped ---
...
func TestHeadersToMessage_GroupSequenceOverflow_F12(t *testing.T) {
	...
	if over.Properties != nil && over.Properties.GroupSequence != nil {
		t.Fatalf("F12: out-of-range group-sequence must be dropped, got %d", *over.Properties.GroupSequence)
	}
	...
	if neg.Properties != nil && neg.Properties.GroupSequence != nil {
		t.Fatalf("F12: negative group-sequence must be dropped, got %d", *neg.Properties.GroupSequence)
	}
}

// --- F13: resource-deleted / link:stolen map to permanent errors ---
...
// --- F6 (regression): a bare-int YAML duration is rejected by the real
// production decode path, not silently read as nanoseconds. F6 is already
// fixed upstream in the shared config/parser hook chain; this guards it. ---
...
func TestPluginOptionsDecode_BareIntDuration_Rejected_F6(t *testing.T) {
	...
	require.Error(t, err, "F6: a bare-int duration must be rejected, not decoded as 30ns")
	require.Contains(t, err.Error(), "bare number")
}

// --- F14: a Sender is unusable after Close and does not re-register ---
...
func TestSender_SendAfterClose_ReturnsClosedSentinel_F14(t *testing.T) {
	...
	require.Error(t, err, "F14: Send after Close must fail")
	...
	sess.mu.Unlock()
	require.False(t, present, "F14: closed sender must not re-enter the session health map")
}

// --- F2: repeated failed settlements on a live link force a rebuild and
...
func TestReceiver_FailedSettlements_ForceLinkRebuild_F2(t *testing.T) {
	...
	require.Equal(t, 1, fl.closeCalls,
		"F2: reaching the failed-settle threshold must force exactly one link rebuild")
	require.Len(t, rec.FindEntries(MetricAMQP10SettleFailed), 2,
		"F2: every failed settlement must emit the observability metric")
}
...
func TestReceiver_FailedSettlements_DurableDropsConnection_F2(t *testing.T) {
	...
	require.Equal(t, 0, fl.closeCalls, "F2: a durable link must NOT be closed (that is UNSUBSCRIBE)")
	mc.mu.Lock()
	closed := mc.closed
	mc.mu.Unlock()
	require.True(t, closed, "F2: durable rebuild must drop the connection so the subscription survives")
	require.Len(t, rec.FindEntries(MetricAMQP10SettleFailed), 1)
}

// --- F3: Close winning the Start race must not leak an immortal monitor
...
// monitor is installed — the exact F3 race window. All other metrics are
...
func TestSession_StartClose_Race_NoImmortalMonitor_F3(t *testing.T) {
	...
	err := <-startErr
	require.Error(t, err, "F3: Start must not return nil for a session closed mid-start")
	require.True(t, errors.Is(err, shared.ErrTransportClosedPermanently),
		"F3: Start error must wrap ErrTransportClosedPermanently, got %v", err)
}
