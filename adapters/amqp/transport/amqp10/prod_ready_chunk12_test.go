...
//
//   - HIGH-1: durable subscriptions silently lose continuity across a
...
//   - HIGH-2: malformed-ingress rejection ignored a FAILED settlement,
...
//   - HIGH-3: a durable receiver's close forces a full connection teardown
...
// --- HIGH-1: durable continuity requires an explicit container_id -------
...
func TestFactory_NewReceiver_DurableWithoutExplicitContainerID_Rejected(t *testing.T) {
	...
	require.Error(t, err, "durable receiver on a generated container_id must fail closed (HIGH-1)")
...
// --- HIGH-2: malformed-ingress rejection must honour failed settlement ---
...
// the malformed-ingress settlement-failure path (HIGH-2) unit-testable
...
func TestReceiverLink_Receive_RejectFailure_ReturnsSettlementError(t *testing.T) {
	...
	require.False(t, errors.Is(err, errIngressRejected),
		"an unsettled delivery must NOT be reported as a handled ingress rejection (HIGH-2)")
...
// --- HIGH-3: durable receiver requires a dedicated session --------------
...
func TestFactory_NewReceiver_DurableOnSharedSession_Rejected(t *testing.T) {
	...
	// The durable receiver has an explicit container_id (HIGH-1 passes) but
	// must still be rejected because it would share the session (HIGH-3).
	_, err = f.NewReceiver(context.Background(), ports.ReceiverSpec{
		ID:     "durable",
		Config: Config{Receiver: ReceiverParams{Address: "topic/orders", DurabilityMode: 1}},
	}, sess)
	require.Error(t, err, "durable receiver may not share a session (HIGH-3)")
	require.True(t, errors.Is(err, shared.ErrInvalidPayload), "got %v", err)
}
...
func TestReceiver_HandleLinkError_Durable_SettlementFailure_TearsDownConnection(t *testing.T) {
	...
	// Exact HIGH-2 shape: a failed reject settlement wraps
...
func TestFactory_NewReceiver_ConfigError_DoesNotLeakReservation(t *testing.T) {
	...
	sess := factorySession(t, f, SessionOptions{
		...
		ContainerID: "bridge-01", // explicit → durable HIGH-1 gate satisfied
	})
...
func TestLowLevelConstructor_PermissiveByDesign(t *testing.T) {
	...
	// Factory's HIGH-1 gate rejects for durable receivers.
	sess := newTestSession()
	...
	// a generated-container_id session (HIGH-1 would reject at the Factory).
	...
	// And it does NOT enforce the dedicated-session contract (HIGH-3): a
	...
	require.Error(t, ferr, "Factory.NewReceiver MUST enforce the durable container_id gate (HIGH-1)")
