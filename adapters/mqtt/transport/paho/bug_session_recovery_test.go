...
// Regression pins for the MQTT session-recovery fixes that live at
// the session/receiver/factory level: (emit-error stranded delivery
// requests recovery), (persistent + clean_start warning), MQTT
// (hostname suffix + persistent warning), (generation-safe breaker
...
// an emit error strands its un-acked delivery — MQTT brokers do not
...
func TestReceiver_EmitErrorRequestsSessionRecoveryForDurableDelivery(t *testing.T) {
	...
	assert.True(t, pending,
		"an emit error on a durable delivery must request bounded session recovery")
...
// boundary: an ephemeral session has no broker redelivery contract —
...
// persistent + clean_start=true silently wipes the broker-side
...
func TestNewSession_WarnsOnPersistentCleanStart(t *testing.T) {
	...
	assert.Contains(t, buf.String(), "clean_start=true on a persistent session",
		"the misconfiguration must warn at construction")
...
// (supersedes MQTT): persistent mode + client_id_suffix=hostname
...
func TestFactory_RejectsPersistentHostnameSuffixWithoutAssertion(t *testing.T) {
	...
	require.Error(t, err, "persistent + hostname must be rejected without an explicit stable-identity assertion")
...
// the CircuitBreakerSender must route admission through the
