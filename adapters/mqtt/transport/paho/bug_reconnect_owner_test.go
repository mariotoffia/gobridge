...
// ═══════════════════════════════════════════════════════════════════════════
// Single owner of reconnect reconciliation
//
// Finding: paho OnConnectionUp reconciled
...
//   Net: broker unsubscribed, no error propagated, finding never fired.
//
...
// tests can drive the reconcile path without a live broker. It is the
...
// TestOnConnectionUp_ResetsActiveSubsBeforeSignal pins the new
...
func TestOnConnectionUp_ResetsActiveSubsBeforeSignal(t *testing.T) {
	...
	s.mu.Unlock()
	if n != 0 {
		t.Fatalf("activeSubs must be reset to empty before SessionConnected, got %d entries", n)
	}
	if !connected {
		t.Fatal("connected must be true after OnConnectionUp")
	}
	...
	if got := fake.subscribeCallCount(); got != 0 {
		t.Fatalf("OnConnectionUp must not Subscribe inline, got %d Subscribe calls", got)
	}
	wait.Silent(t, s.Events(), 100*time.Millisecond)
}

// TestReconnectResubscribe_SingleSubscribeAndReconciledOnce is the
...
// TestReconnectResubscribeFailure_PropagatesViaManagerReconcile is the
...
//
// Contrast TestStaleActiveSubs_ZeroDeltaMasksBrokerRejection: the ONLY
// difference is that resets activeSubs to empty BEFORE signalling, so the
...
func TestReconnectResubscribeFailure_PropagatesViaManagerReconcile(t *testing.T) {
	...
	if err == nil {
		t.Fatal("regression: reconnect re-subscribe rejection must propagate from Reconcile")
	}
...
// TestStaleActiveSubs_ZeroDeltaMasksBrokerRejection documents the pre
...
// plan as TestReconnectResubscribeFailure_PropagatesViaManagerReconcile;
...
// TestReconcile_InitialEmptyPlan_EmitsReconciled covers the sender-only
...
func TestReconcile_InitialEmptyPlan_EmitsReconciled(t *testing.T) {
	fake := &fakeReconcileConn{}
	s := NewSession(SessionOptions{
		BrokerURLs: []string{"tcp://192.0.2.1:1883"},
