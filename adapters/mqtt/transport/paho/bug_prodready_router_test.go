...
// Regression pins for the PROD_READY_ISSUES MQTT review fixes that live in
// the router: MQTT-L1 (poison ack-drop instead of terminal), MQTT-L4
// (recycle-window discards metered), MQTT-L5 (ack-after-reconnect counted).
...
// MQTT-L1: a within-cap publish is not a violation.
...
// MQTT-L1: a failed ack (connection cycled under the drop) must not panic or
...
// MQTT-L4: the recycle-window discard branches — previously the router's
...
// MQTT-L5: an ack whose connection cycled (paho ErrPacketNotFound) maps to
...
// MQTT-L2: before the first Reconcile of a process lifetime (nil plan) a
...
// MQTT-L5 + unsettled accounting: a tracked packet whose ack maps
