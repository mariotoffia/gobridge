...
func TestAutoExtendRetriesTransientThenSucceedsS15(t *testing.T) {
	...
	d := newDelivery(ctx, env, mock, "https://test-queue", "receipt-1", 2, true, nil, nil)
	defer d.stop()
...
func TestAutoExtendInterleavedFailSuccessS15(t *testing.T) {
	...
	d := newDelivery(ctx, env, mock, "https://test-queue", "receipt-3", 2, true, nil, nil)
	defer d.stop()
...
func TestAutoExtendStopsAfterMaxFailuresS15(t *testing.T) {
	...
	d := newDelivery(ctx, env, mock, "https://test-queue", "receipt-2", 2, true, nil, nil)
	defer d.stop()
