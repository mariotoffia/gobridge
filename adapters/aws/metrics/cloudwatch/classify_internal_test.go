...
func TestIsPermanentPutError(t *testing.T) {
	tests := []struct {
		...
		{"expired token is retryable", &smithyErr{code: "ExpiredTokenException", fault: smithy.FaultClient}, false},
		{"invalid client token id is retryable", &smithyErr{code: "InvalidClientTokenId", fault: smithy.FaultClient}, false},
		{"unrecognized client is retryable", &smithyErr{code: "UnrecognizedClientException", fault: smithy.FaultClient}, false},
