...
// transient ErrUnavailable fall-through. This does NOT change drain control
// flow — the drain loop keeps polling on any non-stale error (runtime/outbox
// loop.go), and Complete/Release keep the record durable regardless of class,
// so no message moves differently. What it buys is a DISTINCT, alertable
// signal: the PERMANENT class in logs plus the MetricStoreUnhealthy counter
// tell an operator "retrying will not clear this; free disk / restore the
// file", instead of blending into generic transient-throttle noise. Both the
// primary and any extended result code share the lower 8 bits, so the mask
// catches e.g. SQLITE_READONLY_RECOVERY as READONLY. A human-message fallback
// covers errors flattened across a boundary that lost the typed *sqlite.Error.
...
// This is an OBSERVABILITY signal, not a control-flow change: the drain loop
// keeps polling and the record stays durable either way (nothing branches on
// the store-error class), but ErrCodeInternal + ErrorPermanent surfaces
// "operator must intervene" in logs, and Store.observe pairs the fatal
// classification with the MetricStoreUnhealthy counter so the fault is visible
// on the dashboard.
var errStoreFatal = shared.NewBridgeError(
	shared.ErrCodeInternal, shared.ErrorPermanent,
	"sqliteoutbox: fatal storage fault (disk full, corruption, read-only, or not a database)",
)
...
func mapError(err error) error {
	...
	// fall-through, both of which would keep the bridge retrying forever.
