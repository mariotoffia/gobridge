...
func TestDelivery_Envelope(t *testing.T) {
	...
	d := newDelivery(context.Background(), env, mock, msg, 30*time.Second, false, nil)
...
func TestDelivery_AutoExtend_CallsRenew(t *testing.T) {
	...
	env := &domain.Envelope{ID: "msg-1"}
	msg := &azservicebus.ReceivedMessage{MessageID: "test-msg"}
	d := newDelivery(context.Background(), env, mock, msg, 2*time.Second, true, nil)
...
func TestDelivery_AutoExtend_StopsOnError(t *testing.T) {
	...
	env := &domain.Envelope{ID: "msg-1"}
	msg := &azservicebus.ReceivedMessage{MessageID: "test-msg"}
	_ = newDelivery(context.Background(), env, mock, msg, 2*time.Second, true, nil)
...
func TestDelivery_NoAutoExtend(t *testing.T) {
	...
	env := &domain.Envelope{ID: "msg-1"}
	msg := &azservicebus.ReceivedMessage{MessageID: "test-msg"}
	d := newDelivery(context.Background(), env, mock, msg, 2*time.Second, false, nil)
...
func TestDelivery_MultipleStopsAreSafe(t *testing.T) {
	...
	env := &domain.Envelope{ID: "msg-1"}
	msg := &azservicebus.ReceivedMessage{MessageID: "test-msg"}
	d := newDelivery(context.Background(), env, mock, msg, 30*time.Second, true, nil)
