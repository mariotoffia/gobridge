...
// TestEffectiveStoreCloseGrace_DerivesFromPolicyTimeouts proves the inherited
...
func TestClampedStoreCloseGrace_BoundedByShutdownDeadline(t *testing.T) {
	...
	t.Run("deadline shorter than derived grace clamps to the deadline", func(t *testing.T) {
		...
		assert.Less(t, got, 65*time.Second, "the derived grace must be clamped, not passed through")
