...
func TestRunChain_EmptyProcessors(t *testing.T) {
	err := runtime.RunChain(context.Background(), nil, &domain.Envelope{ID: "1"})
	if err != nil {
...
func TestRunChain_SingleProcessor(t *testing.T) {
	...
	err := runtime.RunChain(context.Background(), []ports.Processor{proc}, &domain.Envelope{ID: "1"})
	if err != nil {
...
func TestRunChain_ProcessorOrdering(t *testing.T) {
	...
	err := runtime.RunChain(context.Background(), processors, &domain.Envelope{ID: "1"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(order) != 3 {
...
func TestRunChain_ProcessorModifiesEnvelope(t *testing.T) {
	...
	env := &domain.Envelope{ID: "1", Subject: "original"}
