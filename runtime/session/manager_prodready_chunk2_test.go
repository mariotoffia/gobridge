...
// source session is closed BEFORE the lease is released (CRITICAL-1).
...
// regression test for CRITICAL-1: an exclusive session that holds the lease and
...
func TestSessionManager_SessionFailure_ClosesSourceBeforeReleasingLease(t *testing.T) {
	...
	if closeAt == 0 {
		t.Fatal("CRITICAL-1: source session was NEVER closed on the session-failure path; " +
	...
	if closeAt >= releaseAt {
		t.Fatalf("CRITICAL-1: source session closed (seq=%d) at or after the lease was released (seq=%d); "+
...
// regression test for HIGH-6: a reconnect-driven Reconcile that ignores ctx must
...
func TestSessionManager_ReconnectReconcile_CtxIgnoringUnblocksAtCeiling(t *testing.T) {
	...
	case <-time.After(3 * time.Second):
		t.Fatal("HIGH-6: Run did not return after the reconcile ceiling fired — a ctx-ignoring " +
...
// for B5. boundedReconcile spawns a fresh goroutine on every reconnect and has no
...
func TestB5_ReconnectReconcile_CtxIgnoring_EscalatesTerminal(t *testing.T) {
	fake := clocktest.NewAt(time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC))
	store := newLeaseLossStore(1<<30, nil) // never fail a renew
	sess := newCtxIgnoringReconcileSession()
	t.Cleanup(func() { close(sess.release) }) // free the parked ctx-ignoring goroutine
	...
	case err := <-runErr:
		if !errors.Is(err, ErrSessionUnrecoverable) {
			t.Fatalf("B5: a ctx-ignoring reconnect Reconcile must escalate to a terminal "+
	...
	case <-time.After(3 * time.Second):
		t.Fatal("B5: Run did not return after the reconcile ceiling fired")
	}
	...
	if got := sess.reconcileCalls.Load(); got != 2 {
		t.Fatalf("B5: expected exactly 2 Reconcile calls (initial + one hung reconnect), got %d — a "+
...
// regression test for B1: when the bounded source Close IGNORES ctx and only the
...
// still-consuming old owner, re-opening the very split-brain CRITICAL-1 closes.
...
func TestSessionManager_SessionFailure_WedgedCloseDoesNotReleaseLease(t *testing.T) {
	...
	case err := <-runErr:
		if !errors.Is(err, ErrSessionUnrecoverable) {
			t.Fatalf("B1: a wedged (ctx-ignoring) Close must escalate to terminal ErrSessionUnrecoverable, got %v", err)
		}
	case <-time.After(3 * time.Second):
		t.Fatal("B1: Run did not return after the bounded-close ceiling fired")
	}
	...
	if n := store.releaseCount(); n != 0 {
		t.Fatalf("B1: lease released %d time(s) after a WEDGED source Close; a standby can now overlap a "+
