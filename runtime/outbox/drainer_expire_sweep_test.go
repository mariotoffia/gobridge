...
// ---------------------------------------------------------------------------
// Production-readiness fixes: (expire-sweep policy gate), (drop-vs-DLQ
// gate on permanent/poison), (OnSettled after Complete), (release-failure
...
// settleCountingHook counts OnSettled invocations so can assert the terminal
...
// newExpireDrainer builds a drainer over the given OnExpired policy for the
...
// with no store, so BOTH legs of the gate (drop policy, no store) hold.
...
// ---------------------------------------------------------------------------
// the bulk expire sweep is deferred for on_expired:dlq routes (so expiry
...
func TestDrainer_ExpireSweep_SkippedForDLQPolicy(t *testing.T) {
	...
	t.Run("drop policy runs the sweep scoped to the partition", func(t *testing.T) {
		...
		if len(got) != 1 || got[0] != "SESSION#sess-exp" {
			t.Errorf("Expire scoped to %v, want exactly [SESSION#sess-exp] (partition-scoped sweep)", got)
		}
	})
}
...
// the bulk sweep would have destroyed.
...
// ---------------------------------------------------------------------------
// permanent and poison terminal handling honors OnPermanentFailure=drop and
...
// ---------------------------------------------------------------------------
// OnSettled fires only AFTER the terminal Complete durably lands. A Complete
...
// ---------------------------------------------------------------------------
// a transient send failure whose subsequent Release ALSO fails (store I/O
...
// ---------------------------------------------------------------------------
// a stale owner must NOT Complete a record after losing the lease. Owner
...
func TestDrainer_TokenGoesStaleBetweenSendAndComplete_DoesNotComplete(t *testing.T) {
	...
	if got := store.completedIDs(); len(got) != 0 {
		t.Errorf("completed = %v, want [] (a stale owner must NOT Complete after losing the lease)", got)
	}
...
// tripped), so the test can sequence both transitions deterministically —
// no sleeps or polls.
...
// ---------------------------------------------------------------------------
// a watchdog-abandoned send goroutine must NOT complete its old claim if
...
func TestDrainer_WatchdogAbandonedSend_LaterReturnsNil_DoesNotComplete(t *testing.T) {
	...
	// that is exactly the regression.
	close(release)
	select {
	case id := <-completeCh:
		t.Fatalf("Complete called for %q after the batch was abandoned; a watchdog-abandoned send must NOT complete", id)
	case <-metrics.duplicateRisk:
		// Fence tripped and counted the duplicate risk: Complete was skipped.
	}

	if got := store.completedIDs(); len(got) != 0 {
		t.Errorf("completed = %v, want [] (a watchdog-abandoned send must never Complete)", got)
	}
	if got := metrics.sum(shared.MetricOutboxDrainStalled, nil); got != 1 {
