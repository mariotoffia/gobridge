...
// TestDrainerStoreOpContextBounded covers the drainer side: the
...
func TestDrainerStoreOpContextBounded(t *testing.T) {
	d := &Drainer{policy: routing.RoutePolicy{SendTimeout: 4 * time.Second}}
	ctx, cancel := d.storeOpContext(context.Background())
	defer cancel()
	dl, ok := ctx.Deadline()
	if !ok {
		t.Fatal("drainer storeOpContext returned a deadline-less context")
	}
