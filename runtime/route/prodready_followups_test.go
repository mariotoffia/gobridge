...
// runtime uses to read each source transport's redelivery count (E5-FU2). The
...
func TestReceiveCountHeaderKeys_WireContract(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if c.got != c.want {
				t.Errorf("wire-key drift: got %q, want %q — an adapter rename or a runtime edit broke the receiveCount contract (E5-FU2); update BOTH the runtime constant and the paired adapter header key + its own pin test", c.got, c.want)
			}
		})
	}
...
// TestUnparseableReceiveCountKey pins the E5-FU3 detector: it must flag a
...
// TestSendDirectHold_UnparseableReceiveCountEmitsSignal proves E5-FU3's
...
// TestBuildOutboxRecords_StampsBridgeSpanForDrain proves OTEL-N3: the
...
func TestBuildOutboxRecords_StampsBridgeSpanForDrain(t *testing.T) {
	...
	t.Run("active span replaces upstream traceparent on the persisted record", func(t *testing.T) {
		...
		if got, _ := messaging.GetHeaderString(recs[0].Snapshot().Headers(), messaging.HeaderTraceParent); got != bridgeTP {
			t.Fatalf("persisted traceparent = %q, want bridge hop %q (OTEL-N3: upstream %q must be replaced so downstream parents on this bridge)", got, bridgeTP, upstreamTP)
		}
