...
// TestCountLessSource_ReplayCapPoisons proves the bridge-owned replay
...
func TestCountLessSource_ReplayCapPoisons(t *testing.T) {
	const cap = 3
...
// TestHungSender_CapsLeakedGoroutinesAndWedges proves the per-binding
...
func TestHungSender_CapsLeakedGoroutinesAndWedges(t *testing.T) {
...
// TestProcessorTimeout_InvokesCircuitBreakerHook proves the chain reports a
// GENUINE processor-timeout abandon (not a shutdown-grace abandon) to the
// route-level circuit-breaker hook exactly once. This is the wiring adds:
...
func TestProcessorTimeout_InvokesCircuitBreakerHook(t *testing.T) {
...
func TestAbandonedProcessorCircuitBreaker(t *testing.T) {
	r := NewRouteRunnerFromConfig(RouteRunnerConfig{
		RouteID: "high4",
		Policy:  routing.RoutePolicy{DeliveryMode: routing.DeliveryDirectHold},
		Sender:  stubSender{},
...
// TestUndeclaredBindingPlan_RejectedBeforeSend proves the direct-hold
...
func TestUndeclaredBindingPlan_RejectedBeforeSend(t *testing.T) {
	dflt := &countingSender{}
	store := &recordingDLQStore{}
	r := NewRouteRunnerFromConfig(RouteRunnerConfig{
		RouteID: "high5",
