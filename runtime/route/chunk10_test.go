...
// TestStripForeignReceiveCounts pins the ingress redelivery-count allowlist (F3):
...
func TestStripForeignReceiveCounts(t *testing.T) {
	...
	t.Run("forged foreign count on a count-less source no longer wins receiveCount", func(t *testing.T) {
		// WHY (F3): an untrusted MQTT device forges an SQS count. Before the strip
...
// TestRenderAddress_UnterminatedPlaceholder pins F6: an opening '{' with no
...
// TestGenericAddressSanity pins F7: when no transport AddressValidator is
