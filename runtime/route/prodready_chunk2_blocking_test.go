...
// ════════════════════════════════════════════════════════════════════════════
// Chunk-2 adversarial-review BLOCKING findings (B2, B3, B4, B5 route-side).
// Deterministic; no time.Sleep. Timing paths use the injected fake clock.
// ════════════════════════════════════════════════════════════════════════════

// ── B2 ──────────────────────────────────────────────────────────────────────

// TestB2_PanicRecovery_CountLessSource_ReplayCapPoisons proves the panic-recovery
...
func TestB2_PanicRecovery_CountLessSource_ReplayCapPoisons(t *testing.T) {
	const capN = 3
	...
	if poisonedAt < 0 {
		t.Fatalf("count-less panic never poisoned within %d deliveries — the panic path retries forever (B2)", capN+5)
	}
	if poisonedAt != capN {
...
// TestB3_TerminalAckFailure_KeepsLedgerEntry proves the replay ledger is evicted
...
func TestB3_TerminalAckFailure_KeepsLedgerEntry(t *testing.T) {
	const capN = 3
	store := &recordingDLQStore{}
	r := NewRouteRunnerFromConfig(RouteRunnerConfig{
		RouteID: "b3",
	...
	// B3: because the terminal Ack FAILED, the ledger entry must be RETAINED so a
	...
	if _, over := r.replayCapReached(env); !over {
		t.Fatal("B3: after a failed terminal Ack a redelivery must be immediately re-capped, but the ledger was cleared")
	}
}
...
// TestB4_CooperativeSenderAtSendTimeout_NotWedged proves the wedge ceiling is
...
// cooperative sender).
func TestB4_CooperativeSenderAtSendTimeout_NotWedged(t *testing.T) {
	...
	if n := clk.TimerCount(); n != 1 {
		t.Fatalf("B4: the wedge ceiling fired at SendTimeout (armed timers=%d, want 1); it must EXCEED SendTimeout "+
	...
	if r.isWedged() {
		t.Fatal("B4: a cooperative sender that returned must NOT wedge the route (nothing leaked to cap)")
	}
}
