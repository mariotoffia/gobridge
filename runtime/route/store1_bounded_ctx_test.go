...
// deadline, proving the runtime bounds the store operation (STORE-1).
...
// TestStore1_PersistReceivesBoundedContext pins STORE-1: the runtime wraps the
...
func TestStore1_PersistReceivesBoundedContext(t *testing.T) {
	...
	if !store.persistHadDeadline.Load() {
		t.Fatal("Persist received a deadline-less context; the store operation is unbounded (STORE-1)")
	}
}
...
// TestStore1_PersistTimeoutRetriesUncountableSource pins the STORE-1×MQTT-CORE-1
// regression fix: a bounded-store DeadlineExceeded is an infrastructure timeout,
...
func TestStore1_PersistTimeoutRetriesUncountableSource(t *testing.T) {
	...
	if del.acked {
		t.Fatal("a store timeout terminally settled an uncountable message — infra timeout must not poison/drop (STORE-1 regression)")
	}
	if !del.retried {
...
// TestStore1_StoreOpContextBounded unit-checks the helper: it always returns a
// deadline, bounded by SendTimeout, falling back to the default when unset.
func TestStore1_StoreOpContextBounded(t *testing.T) {
