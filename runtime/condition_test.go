...
func TestConditionEval_SubjectField(t *testing.T) {
	...
	env := &domain.Envelope{Subject: "orders.new"}
...
func TestConditionEval_HeaderDotField(t *testing.T) {
	...
	env := &domain.Envelope{Headers: map[string]any{"x-tenant": "acme"}}
	ok, err := eval.evaluate(env, newEvalContext())
	if err != nil {
		t.Fatalf("evaluate: %v", err)
	}
...
func TestConditionEval_HeaderDotField_MissingKey(t *testing.T) {
	...
	env := &domain.Envelope{Headers: map[string]any{"other": "val"}}
...
func TestConditionEval_JSONPath_Nested(t *testing.T) {
	...
	env := &domain.Envelope{Payload: []byte(`{"order":{"status":"confirmed"}}`)}
	ok, err := eval.evaluate(env, newEvalContext())
	if err != nil {
		t.Fatalf("evaluate: %v", err)
	}
...
func TestConditionEval_JSONPath_Deep(t *testing.T) {
	...
	env := &domain.Envelope{Payload: []byte(`{"a":{"b":{"c":{"d":"deep"}}}}`)}
	ok, _ := eval.evaluate(env, newEvalContext())
	if !ok {
...
func TestConditionEval_JSONPath_MissingPath(t *testing.T) {
	...
	env := &domain.Envelope{Payload: []byte(`{"order":{"status":"ok"}}`)}
	ok, _ := eval.evaluate(env, newEvalContext())
	if ok {
...
func TestConditionEval_JSONPath_InvalidJSON(t *testing.T) {
	eval, _ := newConditionEval(MatchCondition{Field: "$.x", Operator: OpEquals, Value: "y"})
	env := &domain.Envelope{Payload: []byte(`{broken}`)}
	ok, _ := eval.evaluate(env, newEvalContext())
	if ok {
...
func TestConditionEval_AllOperators(t *testing.T) {
	tests := []struct {
		...
		{"eq_no_match", MatchCondition{"subject", OpEquals, "orders"}, &domain.Envelope{Subject: "users"}, false, false},

		// ne
		{"ne_match", MatchCondition{"subject", OpNotEquals, "orders"}, &domain.Envelope{Subject: "users"}, true, false},
		{"ne_no_match", MatchCondition{"subject", OpNotEquals, "orders"}, &domain.Envelope{Subject: "orders"}, false, false},

		// prefix
		{"prefix_match", MatchCondition{"subject", OpPrefix, "orders."}, &domain.Envelope{Subject: "orders.new"}, true, false},
		{"prefix_no_match", MatchCondition{"subject", OpPrefix, "orders."}, &domain.Envelope{Subject: "users.new"}, false, false},
		{"prefix_exact", MatchCondition{"subject", OpPrefix, "orders"}, &domain.Envelope{Subject: "orders"}, true, false},
		{"prefix_not_contains", MatchCondition{"subject", OpPrefix, "rder"}, &domain.Envelope{Subject: "orders"}, false, false},

		// contains
		{"contains_match", MatchCondition{"subject", OpContains, "admin"}, &domain.Envelope{Subject: "/api/admin/users"}, true, false},
		{"contains_no_match", MatchCondition{"subject", OpContains, "admin"}, &domain.Envelope{Subject: "/api/public"}, false, false},

		// regex
		{"regex_match", MatchCondition{"subject", OpRegex, `^order-\d+$`}, &domain.Envelope{Subject: "order-123"}, true, false},
		{"regex_no_match", MatchCondition{"subject", OpRegex, `^order-\d+$`}, &domain.Envelope{Subject: "user-abc"}, false, false},

		// gt / lt / gte / lte
		{"gt_match", MatchCondition{"header.priority", OpGreaterThan, float64(5)}, &domain.Envelope{Headers: map[string]any{"priority": float64(10)}}, true, false},
		{"gt_equal", MatchCondition{"header.priority", OpGreaterThan, float64(5)}, &domain.Envelope{Headers: map[string]any{"priority": float64(5)}}, false, false},
		{"lt_match", MatchCondition{"header.priority", OpLessThan, float64(5)}, &domain.Envelope{Headers: map[string]any{"priority": float64(3)}}, true, false},
		{"gte_equal", MatchCondition{"header.priority", OpGTE, float64(5)}, &domain.Envelope{Headers: map[string]any{"priority": float64(5)}}, true, false},
		{"lte_equal", MatchCondition{"header.priority", OpLTE, float64(5)}, &domain.Envelope{Headers: map[string]any{"priority": float64(5)}}, true, false},

		// exists
		{"exists_true_present", MatchCondition{"header.x", OpExists, true}, &domain.Envelope{Headers: map[string]any{"x": "v"}}, true, false},
		{"exists_true_absent", MatchCondition{"header.x", OpExists, true}, &domain.Envelope{Headers: map[string]any{}}, false, false},
		{"exists_false_absent", MatchCondition{"header.x", OpExists, false}, &domain.Envelope{Headers: map[string]any{}}, true, false},
		{"exists_false_present", MatchCondition{"header.x", OpExists, false}, &domain.Envelope{Headers: map[string]any{"x": "v"}}, false, false},

		// in
		{"in_match", MatchCondition{"subject", OpIn, []any{"prod", "staging"}}, &domain.Envelope{Subject: "prod"}, true, false},
		{"in_no_match", MatchCondition{"subject", OpIn, []any{"prod", "staging"}}, &domain.Envelope{Subject: "dev"}, false, false},

		// numeric coercion
		{"numeric_string", MatchCondition{"header.count", OpGreaterThan, float64(5)}, &domain.Envelope{Headers: map[string]any{"count": "10"}}, true, false},
		{"numeric_json_number", MatchCondition{"$.priority", OpGreaterThan, float64(5)}, &domain.Envelope{Payload: []byte(`{"priority":10}`)}, true, false},
		{"numeric_non_numeric_err", MatchCondition{"header.x", OpGreaterThan, float64(5)}, &domain.Envelope{Headers: map[string]any{"x": "abc"}}, false, true},
	}
...
func TestConditionEval_UnknownOperator(t *testing.T) {
	...
	_, err := eval.evaluate(&domain.Envelope{Subject: "x"}, newEvalContext())
	if err == nil {
...
func TestConditionEval_NaN_Rejected(t *testing.T) {
	...
	env := &domain.Envelope{Headers: map[string]any{"x": "NaN"}}
...
func TestConditionEval_Inf_Rejected(t *testing.T) {
	eval, _ := newConditionEval(MatchCondition{Field: "header.x", Operator: OpGreaterThan, Value: float64(5)})
	env := &domain.Envelope{Headers: map[string]any{"x": "Inf"}}
	_, err := eval.evaluate(env, newEvalContext())
	if err == nil {
...
func TestConditionEval_RegexInputTooLong_NoMatch(t *testing.T) {
	...
	env := &domain.Envelope{Subject: string(longSubject)}
	ok, _ := eval.evaluate(env, newEvalContext())
	if ok {
