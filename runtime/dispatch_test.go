...
// to drive the DLQ-write-failure redelivery path. WriteMaxAttempts=1 keeps
...
// TestPoisonDLQWriteFailure_RedeliversWithPolicyDelay is the regression: when
...
func TestPoisonDLQWriteFailure_RedeliversWithPolicyDelay(t *testing.T) {
	...
	// core assertion: the redelivery delay is the policy backoff, not 0. A
...
// TestSharedOutboxPersistFailure_ReplayCap_Poisons is the regression for the
...
func TestSharedOutboxPersistFailure_ReplayCap_Poisons(t *testing.T) {
	...
	t.Run("below cap retries with policy backoff", func(t *testing.T) {
		...
		// residual on this path: the persist-failure retry must carry the
...
// TestSharedOutboxUnknownBinding_ReplayCap_Poisons is the regression for the
...
// The ingress strip decides whether a forged foreign count survives to
// receiveCount.
...
// TestForgedIngressReceiveCount_StrippedBeforeReplayCap is the integration
