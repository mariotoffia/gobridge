...
| **MQTT settlement recovery** | Persistent/Exclusive MQTT response to a successful `Delivery.Retry` for QoS 1/2: leave the packet protocol-unsettled, synchronously degrade readiness, stop new ingress, drain accepted settlements for at most 5 s, then rebuild the connection with the same `client_id`, non-zero session expiry, and `clean_start=false`. Credential reload and recovery share one context-aware serialization gate. One hard deadline, reused from the conservative post-acquire activation timing, covers gate wait, drain, disconnect, reconnect, and replacement reconcile. Concurrent requests coalesce; completion state and the 30 s minimum interval are published atomically before readiness can recover. Success requires both CONNACK Session Present and replacement-generation reconciliation; absence fails the Session instance closed below Full readiness. Ephemeral Retry remains `shared.ErrNotSupported`. |
