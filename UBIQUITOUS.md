...
| **MQTT accepted metadata cap** | The retained-memory admission limit applied before adapter enqueue: no more than 128 MQTT User Properties and no more than 128 KiB of encoded topic/properties metadata. A violating QoS 1/2 packet terminally recycles the session instead of leaving an unfinishable acknowledgement-tracker entry. |
| **MQTT ingress memory bound** | Conservative per-session bound calculated by `paho.IngressMemoryBound`: `packet = ceil(maxPacketSize(max_payload_bytes) × 1.25)`, `window = receive_maximum + dispatch_capacity + route_max_in_flight + 1`, `bound = packet × window`. `dispatch_capacity` equals effective `receive_maximum` and is one reservation shared by dispatch and startup/migration pending entries. `maxPacketSize` is the retained-heap base for an accepted MQTT v5 PUBLISH: the separately advertised wire packet limit plus both Paho User Property representations and fixed SDK/queue structures. Every addition/multiplication is checked before execution. |
...
| **AWS MQTT memory profile** | File-based AWS bootstrap policy applied on every initial load and hot reload: reserve 25% of `ContainerMemoryBytes` for consumed MQTT ingress sessions, divide it equally by unique session, and derive each default Receive Maximum with `paho.LargestSafeReceiveMaximum`. `ReservedMemoryBytes` plus the MQTT reservation must leave at least 20% container headroom. Sender-only MQTT sessions consume no allocation. CDK overwrites `ContainerMemoryBytes` from the actual Fargate task memory so runtime accounting cannot diverge from the hard limit. |
