...
| **MQTT settlement recovery** | Persistent/Exclusive MQTT response to a successful `Delivery.Retry` for QoS 1/2: leave the packet protocol-unsettled, synchronously degrade readiness, stop new ingress, drain accepted settlements for at most 5 s, then rebuild the connection with the same `client_id`, non-zero session expiry, and `clean_start=false`. Queueing immediately arms Session Present enforcement. Retry otherwise publishes only queued/degraded request state; active-attempt and target-epoch state become visible after the worker acquires the single context-aware session gate shared by reconcile/reload/recovery. Success requires Session Present evidence and reconcile for the exact target epoch. All terminal causes flow through one idempotent transition; whichever caller wins latches the permanent error and coherently clears recovery state before one bounded teardown and manager signal. Recovery quiescence is explicit per generation and is set only after the settlement barrier succeeds; terminal teardown may skip that barrier only for the exact quiesced generation, so Session Present failure before/during drain cannot permit lease release ahead of accepted work or bounded abort. Future APIs fail terminally. Ephemeral Retry remains `shared.ErrNotSupported`. |
