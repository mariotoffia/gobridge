...
| **DurableSessionIdentity** | Opaque, secret-safe SHA-256 fingerprints returned by an optional typed plugin-config capability. The state fingerprint covers every effective field selecting transport-owned durable broker state; the collision-domain fingerprint covers broker coordinates plus effective client identity. The Supervisor compares them before startup/live reload and never logs raw identity material. |
