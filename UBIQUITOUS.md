...
| **Post-acquire activation budget** | For an Exclusive session, the full time available after Lease acquisition for initial `Start` + `Reconcile` (including durable replay verification) before fail-closed teardown: `LeaseTTL - teardown margin`, measured from the pre-Acquire timestamp. It is distinct from the shorter recurring reconnect-event reconcile cap. Typed transport config may expose effective phase timings through `PostAcquireActivationTimingConfig` so the builder rejects a phase that cannot fit before opening stores/transports. |
