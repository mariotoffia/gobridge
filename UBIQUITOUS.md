...
| **MetricMQTTQoSDowngraded** | MQTT metric `MQTTQoSDowngraded` (`paho.MetricMQTTQoSDowngraded`) counting subscriptions the broker GRANTED at a lower QoS than requested (e.g. requested QoS 2, SUBACK granted QoS 0) — a silent downgrade removes the offline/redelivery guarantee the route assumes and opens a disconnect-gap loss window. Tagged `session_id` (the `client_id`); incremented with a loud warning ONCE per subscription transition (initial subscribe, reconnect, or a requested-QoS change), not per reconcile (`c4-qos-downgrade`). Any non-zero value warrants investigating a broker QoS-cap policy. |
