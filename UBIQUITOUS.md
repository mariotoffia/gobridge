...
| **MQTT ingress memory bound** | Conservative per-session bound calculated by `paho.IngressMemoryBound`: `packet = ceil(maxPacketSize(max_payload_bytes) × 1.25)`, `window = receive_maximum + dispatch_capacity + route_max_in_flight + 1`, `bound = packet × window`. `dispatch_capacity` equals effective `receive_maximum`. `maxPacketSize` is the complete admitted MQTT v5 PUBLISH packet: payload plus fixed-header byte, worst-case four-byte Remaining Length, maximal topic and length prefix, QoS packet identifier, properties-length encoding, and the bounded properties allowance. Every addition/multiplication is checked before execution. |
