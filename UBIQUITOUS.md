...
| **MQTT settlement recovery** | Persistent/Exclusive MQTT response to a successful `Delivery.Retry` for QoS 1/2: leave the packet protocol-unsettled, synchronously degrade readiness, stop new ingress, drain accepted settlements for at most 5 s, then rebuild the connection with the same `client_id`, non-zero session expiry, and `clean_start=false`. Concurrent requests coalesce; completed attempts are at least 30 s apart. Recovery succeeds only when CONNACK has Session Present; absence fails the Session instance closed below Full readiness. Ephemeral Retry remains `shared.ErrNotSupported`. |
