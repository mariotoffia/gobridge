...
| Term | Meaning |
|---|---|
| **Envelope** | The canonical message moving through the bridge. Carries `Subject`, `Headers`, and the immutable fields `ID`, `Payload`, `CreatedAt`, `ExpiresAt` — accessed via read-only accessors (`ID()`, `Payload()`, `CreatedAt()`, `ExpiresAt()`); constructed via `NewEnvelope`/`MustEnvelope`. Trusted transport ingress may use `NewEnvelopeFromImmutablePayload`; `Clone` and the runtime-only `SharePayloadFrom` operation may share immutable payload backing. `Payload()` remains defensive and `SetPayload` replaces the backing copy-on-write. Machine-enforced aggregate root of the messaging context. |
...
| **OutboxRecord** | One persisted envelope plus dispatch metadata, status, claim ownership, and replay counters. Aggregate root. `NewOutboxRecords` constructs a fan-out batch that shares one private immutable envelope snapshot per Envelope identity until serialization; public `Snapshot` access remains isolated. |
...
| **Persisted takeover observation** | Lease-row ownership evidence for one exact liveness tuple: lease key, owner, fencing version, renewed value/shape, expiry value/shape, tuple fingerprint, accumulated unchanged monotonic duration, and CAS generation. Observers add only local monotonic elapsed time since a successful read; every add conditions on the exact tuple and evidence so competing observers cannot double-count. Replacement processes inherit confirmed elapsed evidence without cross-host wall-clock subtraction. This is observation of the existing Lease, not a new aggregate or service. |
...
| **TransportFailoverTimingConfig** | Optional generic typed transport-config capability exposing effective worst-case broker-connect and reconcile durations. A declared Failover SLO fails closed when this capability or either duration is unknown. Paho implements it from effective `connect_timeout` and `reconcile_timeout`. |
...
| **RouteSessionDef.FailoverSLO** | Optional blueprint duration field (`failover_slo`). Its endpoint is failure detection to successor `ServiceLevelFull`; empty/zero in runtime means no objective is declared. Preflight validates `lease TTL + ceil(1.25 × acquire poll) + renew call timeout + broker connect timeout + reconcile timeout + startup allowance <= Failover SLO` before resources are opened. Validation is necessary but measured warm/cold evidence is required before a claim. |
| **RouteSessionDef.StartupAllowance** | Blueprint duration field (`startup_allowance`) reserving explicit process startup work in a declared Failover SLO. Default zero, nonnegative, maximum 10 minutes. |
...
| **IngressMemoryConfig** | Optional session `PluginConfig` capability (`ports.IngressMemoryConfig`) that validates a transport-owned ingress byte bound against the effective aggregate `routeMaxInFlight`. The bridge calls it once per consumed ingress session after dedicated-session cardinality validation and before opening stores or transports; sender-only sessions do not participate. |
| **IngressMemoryProfileConfig** | Deployment extension of `IngressMemoryConfig` (`ports.IngressMemoryProfileConfig`) that accepts a per-session byte allocation and derives safe transport concurrency. Safe explicit values are preserved; unsafe explicit values return `ErrInvalidConfig` and are never silently clamped. |
...
| **MQTT settlement recovery** | Persistent/Exclusive MQTT response to a successful `Delivery.Retry` for QoS 1/2: leave the packet protocol-unsettled, synchronously degrade readiness, then recycle the durable session under one context-aware gate. Queueing arms Session Present enforcement; the active target epoch is published only after gate acquisition. Success requires Session Present evidence and reconcile for that exact epoch. Every terminal cause flows through one idempotent transition, so the first caller latches the permanent error and coherently clears recovery state before one bounded teardown/signal. A generation-scoped drain state (`not-started`, `in-progress`, `finished`) gives exactly one owner the ≤5 s settlement barrier: terminal teardown starts it only when not started, joins its existing done signal while in progress, and disconnects immediately once finished regardless of success/timeout. Concurrent causes cannot duplicate drain, disconnect, or terminal close; Session Present failure cannot permit lease release ahead of the shared barrier or bounded abort. Future APIs fail terminally. Ephemeral Retry remains `shared.ErrNotSupported`. |
...
| **MQTTSessionRecoveryRecycle** | Counter metric and `SessionHealth.RecoveryRecycleCount` for actual settlement-recovery recycle attempts started after the session gate is acquired. Gate timeout/cancellation before recycle does not increment. Metric tag cardinality is bounded to `session_id`; producer/message identity is never a dimension. |
...
| **CapDedicatedIngressSession** | Transport capability `dedicated_ingress_session` (`ports.CapDedicatedIngressSession`) marking a stateful transport whose session-level ingress dispatch and protocol settlement are shared failure domains. The bridge preflight permits at most one logical `Receiver` per such session and at most one consuming route runner for that receiver before opening stores or transport resources; `Sender`s may still share it. Paho MQTT declares it and also reserves the receiver atomically on the `Session`, so factory aliases and concurrent programmatic factory calls cannot bypass the invariant. |
| **MQTT predecode ingress guard** | Adapter-owned `net.Conn` read boundary installed after TLS/WebSocket decoding and before Paho client packet decoding. It frames at most one MQTT wire packet, checks Remaining Length and the advertised Maximum Packet Size before allocation, and structurally validates raw PUBLISH topic/QoS/packet-ID/properties/payload layout. A violation returns one typed, secret-safe adapter cause and enters the existing terminal session lifecycle before the packet can reach Paho acknowledgement tracking or adapter queues. |
| **MQTT accepted metadata cap** | The predecode retained-memory admission limit: at most 128 KiB of raw PUBLISH properties, 128 structurally parsed MQTT User Properties, and 128 KiB of conservative topic-plus-properties metadata. The decoded callback repeats the retained-representation checks as defense in depth. A violation terminally recycles the session instead of leaving an unfinishable acknowledgement-tracker entry. |
| **MQTT ingress memory bound** | Conservative per-session bound calculated by `paho.IngressMemoryBound`: `packet = ceil(decodedPacketSize(max_payload_bytes) × 1.25)`, `crossing = ceil((wirePacketSize + decodedPacketSize) × 1.25)`, `window = receive_maximum + dispatch_capacity + route_max_in_flight`, `bound = packet × window + crossing`. `dispatch_capacity` equals effective `receive_maximum` and is one reservation shared by dispatch and startup/migration pending entries. `decodedPacketSize` includes the accepted wire representation, both Paho User Property representations, and fixed SDK/queue structures. The single crossing term covers the predecode guard's one raw packet plus Paho's concurrent decoded representation; it also exceeds one rejected maximum-wire packet. Every addition/multiplication is checked before execution. |
| **`ingress_memory_budget_bytes`** | MQTT session config key (`SessionOptions.IngressMemoryBudgetBytes`, default `DefaultIngressMemoryBudgetBytes` = 256 MiB) limiting the MQTT ingress memory bound. Exact-bound configurations are accepted; any excess or arithmetic overflow returns `ErrInvalidConfig`. |
| **IngressMemoryStats** | Paho `Session.IngressMemoryStats()` observable snapshot of serialized dispatch queue depth and validated capacity. It exposes counts only, never queued packet contents, and is used by measured backpressure/RSS proofs. |
...
| **AWS MQTT memory profile** | File-based AWS bootstrap policy applied on every initial load and hot reload: reserve 25% of `ContainerMemoryBytes` for every unique started memory-aware session referenced by a `ReceiverDef`, plus every referenced Persistent/Exclusive session that can resume stale backlog; derive each omitted Receive Maximum with `paho.LargestSafeReceiveMaximum`. Route-less receivers/durable sessions contribute zero route concurrency. Ephemeral sender-only sessions with no receiver/subscription are excluded. `ReservedMemoryBytes` plus the MQTT reservation must leave at least 20% container headroom. CDK overwrites `ContainerMemoryBytes` from the actual Fargate task memory so runtime accounting cannot diverge from the hard limit. |
