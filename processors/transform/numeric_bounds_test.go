...
// toInt must reject non-finite and out-of-range floats rather than emitting
...
// (integration): a JSON float too large for int64 rejects under
...
// a payload (non-header) mapping with an empty Target is rejected at New.
...
// a target path that crosses an existing scalar must NOT silently replace
...
// a normal nested target still writes through and creates the intermediate
// object.
...
// a DefaultValue incompatible with its own Transform fails fast at New,
