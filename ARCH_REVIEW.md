...
- **L-3** Move route-graph validation from `config/validate.go` into the existing `validate/` package. *(Clean-Arch F5)* - DONE
...
- **L-5** Add `ports.RuntimeQuery` / `ports.RuntimeCommand` interfaces — only worth doing if a second driving adapter (CLI, gRPC) is on the roadmap. *(Hex F-3)* - DONE
...
- **L-20** Capability strings (`ports/transport.go:152-162` — `CapStatefulSession`, `CapSourceRedelivery`, …) are stringly-typed but at least centralized. Adding a capability requires touching `runtime/validator.go` — discoverable seam, acceptable. Flagged for visibility only. *(AP-030)* - DONE
  **Status:** Resolved 2026-05-12. Verified this is a visibility-only finding: `Capability` is a typed string with centralized constants in `ports/transport.go`, and `runtime/validator.go` remains the discoverable seam for adding capabilities. No production changes were required.
  **What landed:** none — no code changes required.
  **Tests added:** none.
