...
// checkIgnoredHTTPBlock enforces the AWS deployment profile's policy for
...
func checkIgnoredHTTPBlock(logger *slog.Logger, logical *ports.BridgeConfig) error {
	...
	if logical.HTTP.TLSCertFile != "" || logical.HTTP.TLSKeyFile != "" {
		return fmt.Errorf("bootstrap: the AWS deployment profile does not serve in-process TLS " +
	...
	logger.Warn("bootstrap: the AWS deployment profile does NOT honor the bridge config `http:` block; "+
