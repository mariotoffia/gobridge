# deployment/aws — Ubiquitous Language
...
| Term | Meaning |
|---|---|
| **Bootstrap config** | Deployment-owned runtime parameters (`infra.BootstrapConfig`). Static per task revision. Delivered via env var `GOBRIDGE_AWS_BOOTSTRAP_JSON` or file `GOBRIDGE_AWS_BOOTSTRAP_FILE`. Distinct from `ports.BridgeConfig`. |
...
| **Deployment baseline content digest** | Content-only HA baseline identity for both file and DynamoDB sources, using the [project definition](../../UBIQUITOUS.md#deployment--seeding-deploymentaws); committed artifacts retain their actual stored version and full digest. |

## Reload Mechanics

| Term | Meaning |
|---|---|
| **In-place reload** (`applyInPlace`) | Tried first by `applyLogicalConfig`: when `bridge.PlanInPlaceReload` accepts the change, the installed runtime is kept and only the changed reload units are retired and grafted, with parts built over the installed registry's factories. `unchanged` keeps the runtime and rejects the apply; `torn` stops the runtime and runs `recoverPrevious`; `wedged`, or a torn runtime whose stop fails, enters the wedged state. See the [project term](../../UBIQUITOUS.md#cross-cutting-layer-2) and [ADR 0018](../../docs/adr/0018-reload-in-place-by-unit.md). |
| **Swap mode** | Strategy used by `applyLogicalConfig` to replace the running runtime when the change cannot be reloaded in place. |
| **Overlap swap** (`swapModeOverlap`) | Build + Start new runtime, install, then Stop old. Default. |
| **Prepare/commit swap** (`swapModePrepareCommit`) | Used when `bridge.RequiresSerializedSwap` finds an exclusive broker identity in the incoming config, or held by the running config on a transport the incoming one still uses — declared exclusivity, an advertised `ports.CapExclusiveIdentity`, or a factory reporting it from a receiver config (an exclusive amqp091 consumer, a Service Bus receiver pinned to one `session_id`). Stop old → `Complete` → Start new → install. |
...
| **Recover previous** | `recoverPrevious(ctx, oldApplied)` — best-effort rebuild from last-good applied config when a prepare/commit swap fails mid-flight, or an in-place reload ends torn. |
...
| **gobridge package** | `cdk/gobridge`, the one package a consumer imports. Re-exports the facades as `NewSingle`, `NewCluster` and `NewHA`, plus config, image, EFS, alarm, ALB and lookup helpers and aliases for their prop types. |
| **BridgeConfigSource** | Sealed synth-time config input to all three facades, `gobridge.Config`. `ConfigFile(path)` reads a local document; `ConfigInline(*ports.BridgeConfig)` supplies typed config. Both drive validation, grants, and optional Go-build embedding without a separate S3 config asset. Distinct from the bootstrap-selected runtime config source. |
| **BridgeImageSource** | Sealed image input to all three facades, `gobridge.Image`, re-exported from `cdk/internal/imgsource.Source`. Nil selects `ImageFromGoBuild` with empty props. `ImageFromRegistry(ref)` preserves a digest-pinned reference; `ImageFromEcr(repo, tag)` uses a consumer-managed ECR tag or digest; `ImageFromGoBuild(props)` stages an embedded Dockerfile for a published command. The internal constructors are `NewRegistry`, `NewEcrRepository`, and `NewGoBuild`; `Materialize` creates the CDK image and `RuntimePlatform` aligns the Fargate task with it. |
| **ImageGoBuildProps** | Build settings, `gobridge.GoBuild`, re-exported from `imgsource.GoBuildProps`: published lib-module `Version` (empty takes the `cdk` version the app depends on), optional command `Package`, `BuildTags`, digest-pinned `GoImage` and `BaseImage`, and `Platform` (`linux/amd64` by default, or `linux/arm64`). The command must support this profile's bootstrap and health check. |
...
| **Profile base set** | Plugin families always linked into `gobridge-aws`: AWS, MQTT, native stores, and HTTP. AMQP 0-9-1, AMQP 1.0, and Azure Service Bus are additive `gobridge_amqp091`, `gobridge_amqp10`, and `gobridge_azure` tags; `gobridge_all` selects all three optional families. OTel is not wired by this profile. See [PLUGIN.md](../../PLUGIN.md#binary-composition-build-tags). |
| **BridgeConfigSource (current materialization)** | Supersedes the earlier S3-asset description: `ConfigFile` reads a local document and `ConfigInline` supplies typed config; both drive validation and grants, and `ImageFromGoBuild` embeds the parsed result without a separate S3 config asset. |
...
| Term | Meaning |
|---|---|
| **Queues** | Construct prop `map[string]awssqs.IQueue`, keyed by the queue name the bridge config references. No auto-import scanning. Missing entry → `addError`: `add Queues["X"] = queue`. |
| **QueueTags** | Construct prop `map[string]QueueTags{Tags, NamePrefix}`, keyed like `Queues`, selecting a listed queue by tag through `BindQueueTags`. |
| **Secrets** | Construct prop `map[string]awsssm.IParameter`, keyed by SSM path or `pms://` URI (e.g. `/bridge/mqtt`). Missing entry → `addError`: `add Secrets["/path"] = param`. |
| **QueueRegistry** | `string→awssqs.IQueue` map the facades build from `Queues` and `QueueTags` with `registry.FromProps`. Tier B resolves YAML SQS-by-name references via `QueueRegistry.Ref(name) → QueueRef`; `bridgecfg` takes the same refs. Lives in `cdk/registry/`. |
| **SsmParamRegistry** | `string-URI→awsssm.IParameter` map the facades build from `Secrets`. Tier B resolves YAML SSM URI references via `SsmParamRegistry.Ref(uri) → ParamRef`. Lives in `cdk/registry/`. |
...
| Term | Meaning |
|---|---|
| **BridgeRef** | Consumer-side handle returned by `gobridge.Lookup`. Exposes the same accessor surface (`AdminURL`, `HealthzURL`, optional ARNs) as the producing constructs but resolves values lazily through SSM tokens. |
| **LookupBridge** | `gobridge.Lookup(scope, id, ssmPrefix)`, wrapping `gobridgecdk.LookupBridge`, returning a `BridgeRef`. Reads `<prefix>/admin-url`, `<prefix>/healthz-url`, `<prefix>/manifest-version` (and optional `<prefix>/alb-arn`, `<prefix>/cluster-arn`, `<prefix>/efs-id` if producer used `IncludeARNs()`). Values use `awsssm.StringParameter_FromStringParameterName` deploy-time tokens. EFS presence uses an optional, context-cached `ValueFromLookup`; `EfsID()` is nil until presence resolves or when the producer has no EFS. Refresh that lookup after changing filesystem use. Manifest-version sentinel allows future schema breaks to fail consumer synth fast. |
