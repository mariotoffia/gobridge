...
// queueMap lists a builder registry's queues the way the construct props take
// them, so a fixture keys both the config references and the grants by one name.
func queueMap(reg *registry.QueueRegistry) map[string]awssqs.IQueue {
	queues := make(map[string]awssqs.IQueue)
	for _, name := range reg.Names() {
		queues[name] = reg.Ref(name).Queue()
	}
	return queues
}
