...
// deployment/aws/UBIQUITOUS.md. Only its factories can supply
...
type GoBuildProps struct {
	// Version is a published lib-module version, for example v0.4.0. Empty uses
	// the version of this CDK module the app depends on, so the image always
	// matches the constructs; an app built against a local replace must set it.
	// Branch names and mutable queries such as main/latest are not accepted.
	Version string
	// Package defaults to the published lib/cmd/gobridge-aws command.
	Package string
...
// OrDefault returns src, or a Go build of the published command at this
// module's own version when src is nil.
//
//nolint:ireturn // Source is sealed; see the package contract.
func OrDefault(src Source) Source {
	if src == nil {
		return NewGoBuild(GoBuildProps{})
	}
	return src
}
...
func normalize(props GoBuildProps) GoBuildProps {
	if props.Version == "" {
		version, err := moduleVersion(debug.ReadBuildInfo())
		if err != nil {
			panic("gobridgecdk: ImageFromGoBuild Version is empty and cannot be taken from the app: " +
				err.Error() + "; set Version to a published release such as v0.4.0")
		}
