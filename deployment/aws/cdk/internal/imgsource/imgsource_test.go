...
func TestGoBuild_RendersPublishedPackageAndHardenedRuntime(t *testing.T) {
	...
	require.Contains(t, dockerfile, "github.com/mariotoffia/gobridge/deployment/aws/lib/cmd/gobridge-aws@v0.4.0")
	...
	require.Contains(t, dockerfile, `ENTRYPOINT ["/usr/local/bin/gobridge-aws"]`)
	require.Contains(t, dockerfile, `CMD ["/usr/local/bin/gobridge-aws", "-healthcheck"]`)
