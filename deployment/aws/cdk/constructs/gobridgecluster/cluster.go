...
// GoBridgeEfsConfig type access). The cdk/gobridge package re-exports
// the public surface; consumers should normally use that rather than
...
func NewGoBridgeCluster(scope constructs.Construct, id *string, props *ClusterProps) *GoBridgeCluster {
	...
	if err != nil {
		panic(fmt.Sprintf("GoBridgeCluster: %v", err))
	}
...
func validateClusterProps(p *ClusterProps) {
	...
	if p.BridgeConfig == nil {
		panic("GoBridgeCluster: BridgeConfig is required (use gobridge.ConfigFile / ConfigInline)")
	}
