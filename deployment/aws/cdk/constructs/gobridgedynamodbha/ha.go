...
import (
	...
	"github.com/mariotoffia/gobridge/deployment/aws/cdk/constructs/internal/grants"
...
type DynamoDBHAProps struct {
	...
	// Image is the image every task runs. Nil builds the published command at
	...
	// queue. Every task is granted exactly these queues.
	...
	// pms://app/key) to its CDK parameter, so every task may read it.
...
func NewGoBridgeDynamoDBHA(scope constructs.Construct, id *string, props *DynamoDBHAProps) *GoBridgeDynamoDBHA {
	...
	if err != nil {
		panic(fmt.Sprintf("GoBridgeDynamoDBHA: %v", err))
	}
