...
func TestRunPhase2_SQSReceiver_MissingFromRegistry_EmitsError(t *testing.T) {
	...
	if !containsAll(t, got, "SQS queue \"foo\"", "is not in Queues", "Queues[\"foo\"]") {
...
func TestRunPhase2_AWSSQSAlias_MissingFromRegistry_EmitsError(t *testing.T) {
	...
	got := errorMessages(t, stack)
	if !containsAll(t, got, "SQS queue \"alias-queue\"", "is not in Queues") {
		t.Fatalf("aws.sqs alias bypassed Queues validation: %v", got)
	}
}
...
func TestRunPhase2_SQSSender_MissingFromRegistry_EmitsError(t *testing.T) {
	...
	got := errorMessages(t, stack)
	if !containsAll(t, got, "SQS queue \"orders-out\"", "is not in Queues") {
		t.Fatalf("missing expected text in errors: %v", got)
	}
}
...
func TestRunPhase2_SQSRef_NilQueueRegistry_EmitsRequiredPropError(t *testing.T) {
	...
	got := errorMessages(t, stack)
	if !containsAll(t, got, "no Queues were supplied", "Queues prop", "\"foo\"") {
...
func TestRunPhase2_SSMURI_NilSsmParamRegistry_EmitsRequiredPropError(t *testing.T) {
	...
	got := errorMessages(t, stack)
	if !containsAll(t, got, "no Secrets were supplied", "Secrets prop", "\"/bridge/mqtt\"") {
		t.Fatalf("missing required-prop error text: %v", got)
	}
	if len(got) != 1 {
...
func TestRunPhase2_ManagedSubscriptionStoreSSMURI_IsCollected(t *testing.T) {
	...
	got := errorMessages(t, stack)
	if !containsAll(t, got, "no Secrets were supplied", "/bridge/managed-store") {
