...
func checkSQS(scope constructs.Construct, cfg *ports.BridgeConfig, reg *registry.QueueRegistry, emit func(string)) {
	...
	if reg == nil {
		...
		if len(refs) > 0 {
			emit(fmt.Sprintf("SQS reference(s) %s need the Queues prop, but no Queues were supplied. "+
				"Add each queue to Queues under its physical name, and a QueueTags entry for tag selectors.",
