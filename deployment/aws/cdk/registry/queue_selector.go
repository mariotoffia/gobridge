...
func (r *QueueRegistry) ResolveQueue(cfg sqs.Config, runtimeScope ...constructs.Construct) (QueueRef, error) {
	...
	if r == nil {
		...
		if cfg.QueueTags != nil {
			return QueueRef{}, fmt.Errorf("SQS queue_tags needs its queue in the Queues prop and its selector in QueueTags; no Queues were supplied")
		}
		return QueueRef{}, fmt.Errorf("SQS reference needs its queue in the Queues prop; no Queues were supplied")
	}
	names := r.Names()
	sort.Strings(names)
	var match QueueRef
	...
	for _, name := range names {
		...
		if match.IsResolved() && *match.Queue().QueueArn() != *ref.Queue().QueueArn() {
			return QueueRef{}, fmt.Errorf("SQS queue reference is ambiguous: it matches more than one queue in Queues; use a unique queue_tags selector or physical name")
		}
		match = ref
	}
	if match.IsResolved() {
	...
	if cfg.QueueTags != nil {
		return QueueRef{}, fmt.Errorf("SQS queue_tags matches no QueueTags entry; add the queue to Queues and its selector to QueueTags under the same key")
	}
	if r.Has(cfg.QueueName) {
		return QueueRef{}, fmt.Errorf("SQS queue %q is a Queues key, not its physical QueueName; key the queue by its physical name or select it with QueueTags", cfg.QueueName)
	}
	return QueueRef{}, fmt.Errorf("SQS queue %q is not in Queues; add Queues[%q] = queue, keyed by that physical QueueName", cfg.QueueName, cfg.QueueName)
}
