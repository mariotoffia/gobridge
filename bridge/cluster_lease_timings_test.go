...
// ---------------------------------------------------------------------------
// Finding (cluster chunk, MEDIUM): binding-only (Path-2) sessions must
...
func TestBindingSessionConfig_InheritsRouteLeaseTimings(t *testing.T) {
	...
	assert.Equal(t, 45*time.Second, sc.LeaseTTL,
		"binding session must inherit the route's lease_ttl: "+
...
// (contract) instead of keeping DefaultConfig's pinned 110s.
func TestBindingSessionConfig_NoRouteSession_LeavesRenewIntervalDerived(t *testing.T) {
	...
	assert.Equal(t, time.Duration(0), sc.RenewInterval,
		"RenewInterval must stay zero (derive-from-TTL downstream, contract)")
	assert.Equal(t, 360*time.Second, sc.LeaseTTL)
}

// ---------------------------------------------------------------------------
// Finding (cluster chunk, LOW): endpoint-resolution failure must not
