...
- [ ] Add the deployed confirm-window phase. **The phase is written and runs; its own assertion does not pass, and what it found is a product defect rather than a gap in the proof.** The lever works exactly as designed — a subscription asking for a QoS the broker caps below it is acked 3/3, applied by all three members, and refused by the broker — but the cohort CONFIRMS it instead of reverting, because on this profile every member's ingress session is deferred-connect and lease-held and a deferred-connect session without the lease is excluded from the readiness fold. All three members therefore record convergence over an empty set of sessions, a minute and a half before the broker's verdict arrives. That is NEW-HIGH-21. The phase records it in place and skips, so its revert assertion starts running the day the convergence signal stops being vacuous.
...
- **Accept when** a deployed cohort has taken a change it accepted and could not run back to its last confirmed generation, and a member that does not vote says why in a surface an operator can read. **The second half is met and proved on the deployed cohort; the first is not, and the reason is NEW-HIGH-21 rather than anything missing from this chunk.**
...
- [ ] **SQLite stores on a deployed task. Attempted; cannot be closed locally, and the reason is now measured twice.** Standing one up under the MQTT topology was refused at synth — the profile requires a SQLite store path to be under the config mount, correctly, because a store on the container's own filesystem is lost on every task replacement — and moving it onto the mount was then refused by the store itself: `managed subscription SQLite parent component "gobridge" is writable by group or other`. The harness bind-mounts a host directory `0777` where the EFS access point creates `755` owned by the container user, and a bind mount is not uid-mapped on every Docker host, so the mount's ownership is not reproducible portably. Closing it is harness work of its own; the credentialed suite can stand it up today. Recorded in `docs/aws-deployment/local-deployment-suite.md`.
...
- [ ] **Lambda either side of the bridge. Not started.** Probe P3 has still never been run, so the topology stays credentialed and `testutil/SPEC.md` §7 is unchanged.
- [x] Run `make test-local-deploy`, then `make lint` and `make test`.
...
- **Accept when** the untested list in that page is empty. **NOT met:** two of the three rows are still open — SQLite with a measured reason that names the harness work needed, Lambda with no work done at all.
...
- [ ] **Prove it on a deployed task. Blocked, measured.** A `direct_hold` MQTT route needs a durable session; a durable session with subscriptions needs `stores.managed_subscriptions` (ADR 0003), which is a property of the durable session the redelivery precondition demands rather than of the delivery mode; and neither store the profile offers can be stood up in a local single-task topology — SQLite is refused twice over (see Chunk 30) and DynamoDB is provisioned only by the HA facade. The claim is proved at every layer a local run can reach and waits on either the SQLite row of Chunk 30 or the credentialed suite.
...
- **Accept when** an MQTT→SQS route deploys and carries messages with no outbox, no lease and no managed-subscription store, and a subscription that genuinely cannot redeliver is still refused. **Partly met:** the refusal half is proved, and the outbox and lease do go; the managed-subscription store does NOT, because the durable session itself requires it, and the deployed run is blocked as above.
