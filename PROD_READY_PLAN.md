...
| 26 | blocked | none — closes the BLOCKER-1 deployment residual whose primary mapping stays with Chunk 17, so the mapping count is unchanged |
...
- **Landed, except the Exit gate: the harness runs and the cohort deploys; the barrier does not converge, for a reason only this run could find.** `make test-local-deploy` deploys the `aws-filebased-config` profile with `cdklocal` against local emulation and drives it: three single-task slots, distinct restart-stable `member_id`s read out of each slot's OWN task definition (not out of a service name), one membership epoch, one generation-zero baseline digest, four deployed tables mirrored to DynamoDB Local, and a live-safe change proposed on the control slot through the admin config transaction API. `RequireSandbox` gained one local branch, `DeployStack`/`DestroyStack` swap the CLI and keep the outputs-file contract, and the rollout observation helpers moved into a shared `rollout_probe.go` parameterised by a probe, so the credentialed proof and the local one assert the same things about the same protocol.
- **The proof stops at convergence, and that is the finding.** Every member computes a different candidate digest than the proposer for the same change, so nobody can join the in-flight rollout and it deadline-aborts at 1/3 acks. Reduced to one field: a YAML round trip turns a nil slice into an empty one (`"broker_urls":null` → `"broker_urls":[]`), so the canonical projection is not a fixed point across persist-and-re-read. Recorded as `NEW-BLOCKER-1` with a reproducer that needs no deployment. Fixing it is its own work — the three candidate fix sites differ in blast radius and the choice belongs with someone holding the barrier's identity contract — so the restart, rollback and confirm-window phases of the cohort test are written and runnable but have never been reached.
- **Three more defects the run found, two of them in shipped code.** The seeder's pinned image ships no PyYAML, so `seeder.sh` exits 50 and no deployed member ever gets a config — the profile could not have worked on real AWS either (`NEW-BLOCKER-2`; the repo's own seeder Dockerfile now installs and verifies it, and the two docs that claimed otherwise are corrected). The credentialed HA fixture could never synthesize (outbox `stale_claim_duration` below the route send timeout) and, once that was fixed, could never boot (`on_expired`/`on_permanent_failure` defaulting to a DLQ the profile does not provision) — both were vetted but never executed (`NEW-TEST-2`). The barrier's "a DIFFERENT rollout is already in flight" error now names both digests, because without them an operator cannot tell a genuinely different change from this defect.
- **Five emulator gaps, each written down beside what it costs.** Four are closed in the harness: CloudFormation drops task-definition volumes and mount points, so the harness re-registers exactly what the assembly declared — which also means the running tasks are one revision off the ones CloudFormation made, so a local run does NOT prove the declared definition reaches ECS intact; ECS does not honour `efsVolumeConfiguration`, so the assembly's EFS volume becomes a host bind mount of one directory; custom-resource handlers always report to `https://<host>:443`, which the emulator does not serve, so a TLS listener shares its network namespace; and there is no task metadata service, so one is served that answers each caller its own address. The fifth cannot be closed: the emulator has no container-dependency model — it neither stores `dependsOn` nor honours it — so a local member can start before its seeder has written, exit, and be replaced until the document is there. The cohort still settles, but the seeder GATE is not exercised locally and no claim may rest on start ordering. `testutil/PLAN.md` §6 records the probe answers.
...
- **Not done:** the Exit gate's executed propose → commit → convergence → restart → rollback, blocked on `NEW-BLOCKER-1`. Nothing published claims otherwise.
