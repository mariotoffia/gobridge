...
- [ ] Prove the confirm window on the same cohort: a member that cannot converge reverts the WHOLE cohort to the last confirmed generation rather than leaving it split. **Not done** — removing a member makes it fail to ANSWER, which the barrier resolves earlier and differently; what runs instead proves that a change one member cannot take is applied by nobody.
...
### Chunk 29: Confirm-window proof and the member that does not vote

- **Issues:** NEW-TEST-3 (the confirm window has no deployed proof), NEW-MEDIUM-26 (a subscription change is acked only by its proposer).
...
- **Why it is its own chunk:** the attempt in Chunk 28 established that neither remaining lever is reachable today, so this needs a change to the product surface, not another test. That makes it work with its own design decision rather than an open box on a landed chunk.
- **What Chunk 28 established, so this does not start from the same guess.** Convergence is decided by SESSION readiness — every non-standby session connected and its declared subscriptions satisfied — so a sender whose target does not exist cannot withhold it, and the work item that proposed one was wrong. Of the two things that CAN withhold it, a session's transport options are a typed plugin config the admin overlay's wire form does not carry at all (`ports.SessionDef.Config` is `json:"-"`), and a receiver's subscription list — the one lever the overlay does carry — was acked only by the member that proposed it.
- [ ] Establish why a member does not vote on a subscription change: whether it refuses to evaluate it, fails to record the vote, or needs the lease to evaluate it at all. The same cohort acks a `bridge.log_level` change 3/3 immediately before and after, so the ack path itself works.
- [ ] Make a member's failure to vote observable. Today the rollout row records only that the barrier ran out of acks, so an operator sees `rollout deadline exceeded with 1/3 acks` and has nothing that names the member or the reason — the same blindness the proof ran into.
- [ ] Decide and document whether a subscription change is live-safe on a managed session. If it is not, the barrier must say so at the vote rather than time out.
- [ ] Once a change exists that every member votes for and none can converge on, add the deployed confirm-window phase to the local cohort: gate on an observed PROVISIONAL commit first (a cohort that never accepted reads identically to one that reverted), then require the settled state to be `reverted` rather than `aborted` — that is the only thing separating the confirm window from the vote-abort path the cohort test already proves.
- [ ] Run `make test-local-deploy`, then `make lint` and `make test`.
- [ ] Accept when a deployed cohort has taken a change it accepted and could not run back to its last confirmed generation, and when a member that does not vote says why in a surface an operator can read.
- **Suggested commit title:** `prove the confirm window on a deployed cohort`


### Chunk 30: The three deployment shapes the matrix has not stood up
...
- **Dependencies:** Chunk 28 for the harness. The Lambda item additionally needs `testutil/SPEC.md` extended — probe P3 answers only whether a Go function runs, not what a Lambda topology's fixture, wiring and assertions look like.
- **Why these three, and why they are not in Chunk 28.** Chunk 28 proved the shapes the profile deploys by default. These are the ones it does not: a store choice (SQLite), a config source (DynamoDB), and a compute shape (Lambda). Each is documented and supported, and each would fail silently in a way only a deployed run would show — which is exactly the class of defect the matrix already caught twice.
- [ ] **SQLite stores on a deployed task.** Every local run so far uses in-memory stores or DynamoDB; the SQLite outbox, DLQ and managed-subscription stores are exercised only in unit and integration tests, never on a deployed task with a real mount. The one thing a deployment adds is the filesystem: the store refuses a parent directory that is not owned by the process user with mode 0700, which is a property of the container image and the mount, not of the store. Stand up a single-task topology on SQLite stores and drive the data plane through it.
- [ ] **Config held in DynamoDB.** `docs/configuration-overview.md` §Layered Configuration and `docs/config-stores.md` document a base file plus a DynamoDB overlay as a supported pattern, and the DynamoDB loader has both poll and streams change detection. No deployed run uses it, and the `aws-filebased-config` profile wires a single `file` layer with no overlay, so the pattern is reachable only through the programmatic API today. Decide whether the profile should expose an overlay layer; if it should, add it and prove a config change made in DynamoDB reaching a deployed task. If it should not, say so where an operator reading those two pages will see it.
- [ ] **Lambda either side of the bridge.** Probe P3 has never been run, so the topology stays credentialed and untested. Run P3 first (a `provided.al2023` function actually invoked through an SQS event source mapping), then extend `testutil/SPEC.md` §5 and §8 with what the topology needs that the ECS one did not — how a function's code is packaged for the emulator, how its event source mapping is asserted, and what a closed producer→bridge→consumer loop asserts on. Only then write the test.
- [ ] Run `make test-local-deploy`, `make test-integration`, then `make lint` and `make test`.
- [ ] Update the matrix and the untested list in `docs/aws-deployment/local-deployment-suite.md` for each item closed, and record the measured reason for any that cannot be.
- **Accept when** the untested list in that page is empty: every entry either has a passing deployed test or a reason, measured rather than assumed, that it cannot have one locally.
- **Suggested commit title:** `close the untested deployment shapes`


### Chunk 31: Gate `direct_hold` on source redelivery, not on visibility extension
...
- **Dependencies:** none. It is independent of the deployment matrix, which is only where it was noticed.
- **The argument, so it is not re-litigated.** `direct_hold` settles the source after the destination accepts. Its precondition is therefore "the source can be left unsettled and will redeliver", not "the source can have a visibility window extended". MQTT meets the first: the adapter acknowledges manually, so the PUBACK is withheld until the delivery settles, and a QoS 1 subscription on a persistent session is redelivered after a crash. The validator asks the second question and so refuses it. The result is that an MQTT→SQS deployment must run an outbox, a lease and a store it does not need — three systems in series where two would do, for a crash window that is identical either way.
- [ ] Replace the `CapVisibilityExtension` precondition with one expressing source redelivery, keeping the HTTP-endpoint branch.
- [ ] Have the MQTT receiver declare that capability per ROUTE rather than per transport, and only when the subscription QoS and the session's persistence actually provide redelivery — a QoS 0 subscription, or `clean_start: true` with no session expiry, must still be refused. `ports/factories.go:157-165` is the existing mechanism for exactly this.
- [ ] Pin both directions: a persistent QoS 1 MQTT route builds on `direct_hold`; a QoS 0 or clean-start one is still refused with a message naming which of the two it failed.
- [ ] Prove it on a deployed task: the local MQTT topology currently carries an outbox, a lease and a managed-subscription store solely to satisfy this gate. It should need none of them.
- [ ] Re-check that the guidance and the validator now agree — the docs were corrected ahead of the code, so they disagree until this lands.
- **Accept when** an MQTT→SQS route deploys and carries messages with no outbox, no lease and no managed-subscription store, and a subscription that genuinely cannot redeliver is still refused.
