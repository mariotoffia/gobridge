...
### Chunk 32: A plan-driven ingress session needs a manager a `direct_hold` route can give it
...
- [ ] Decide what manages a plan-driven ingress session for a route that owns no outbox and holds no lease. The receiver's own binding to the session is the obvious candidate — it is the relationship that makes the subscriptions matter — but it has to be reconciled with the existing two paths rather than added beside them, or a session ends up with two managers.
- [ ] Make the refusal name the new way out. Today it offers only the two an operator cannot use here.
- [ ] Restore the local MQTT topology to a durable ingress session on `direct_hold`, with the SQLite managed-subscription store the durable session owes the broker, and assert BOTH that the deployed task reports the mode and that messages cross the route. The revert of that fixture is in the history; the mount ownership it needs is already fixed.
- [ ] Run `make test-local-deploy`, then `make lint` and `make test`.
- [ ] Update `docs/aws-deployment/local-deployment-suite.md`: the `direct_hold` delivery-mode row and the SQLite row of *Not yet stood up* close together or not at all.
- **Accept when** an MQTT→SQS route deploys with no outbox, no lease and no outbox partition, carries messages, and its SQLite managed-subscription store lives on the deployed mount.
...
- [ ] Run probe P3 (`testutil/SPEC.md` §7): a `provided.al2023` function actually invoked through an SQS event source mapping. It has never been run, so nothing below is committed to until it answers.
- [ ] Extend `testutil/SPEC.md` §5 and §8 with what the topology needs that the ECS ones did not — how a function's code is packaged for the emulator, how its event source mapping is asserted, and what a closed producer→bridge→consumer loop asserts on.
- [ ] Write the test, then run `make test-local-deploy`, `make lint` and `make test`.
- [ ] Update the D5 row of `docs/aws-deployment/local-deployment-suite.md` and of `testutil/SPEC.md` §8.
...
### Chunk 34: The last two residuals of the rollout work
...
- **Dependencies:** none.
- [ ] **A cohort deployed before the artifact fix still holds an undecodable committed config.** The fix is forward-only: the next successful commit rewrites the artifact, so a rolling upgrade clears it while at least one member is still up, and a cohort whose members are ALL down in that window needs the artifact cleared by hand. Decide whether that is worth code (a boot-time decode failure that clears an artifact older than the running binary, under an explicit opt-in) or a release note, and do whichever. **Drop the item entirely if no cohort predating the fix exists** — it cannot affect a first deployment.
- [ ] **`cluster.rollout: independent` inherits the barrier's cohort-shape rules.** The delta classifier is shared, so a change to `bridge.cluster.members` is replacement-required under `independent` too — where the roster is unused and counts nothing. It is stricter than necessary, not unsafe. Either narrow the rule to the modes that have a barrier, or state in `docs/cluster/README.md` §Setup 3 that the roster still may not change live.
- [ ] Run `make lint` and `make test`.
- **Accept when** neither item needs to be remembered: each is closed, coded, or written down where an operator meets it.
