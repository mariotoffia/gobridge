...
A genuine **no-op re-emit** is detected *before* the guard and stays accepted; it is not a reconfiguration. A re-emit is a no-op when the new document has the running configuration's *content normal form* (`ports.ContentNormalForm`, ADR 0016), not only when its bytes are identical: a raised `version` number, sessions or routes listed in another order, a duration spelled differently, or a written-out `bridge.drain_timeout` default all leave every session connected. The new document is adopted as the applied configuration, so the reported `config_version` follows the source while the runtime is kept.
