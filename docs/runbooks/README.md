...
| [Persistent MQTT managed-filter migration](mqtt-managed-subscription-migration.md) | Remove durable wildcard/shared filters safely; inspect, then redrive or purge the `SUBSCRIPTION_REMOVED` dead-letter records a removed filter leaves; without a dead-letter store, restore, drain, and retry when a broker pins an unacknowledged shared delivery. |
