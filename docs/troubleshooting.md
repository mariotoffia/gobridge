...
| `MQTTReceiverEmitRejected` | The route pipeline refused a delivery at emit — a shutting-down or wedged route runner (`Receiver.countEmitRejection`). Tagged `outcome=recovering` for a durable QoS 1/2 delivery (left un-acked; the bounded session recycle makes the broker redeliver it) or `outcome=lost` for QoS 0 (no ack to withhold, no redelivery contract). | A rising `recovering` rate is the leading indicator of the session recycles that follow, each of which redelivers every unsettled delivery on the session. Any non-zero `lost` count is acknowledged best-effort loss — alert on it if QoS 0 ingress is significant to the deployment. |
