...
| `confirm_window` | duration | no | unset (base protocol) | Opts a coordinated rollout into the confirm window: a positive duration makes every commit PROVISIONAL. Each member swaps and must then reach convergence; the coordinator confirms once the whole cohort converged, and if confirmation never lands every member reverts to the last confirmed generation. Unset is the base protocol, where a commit is final; `0s` and other non-positive values are rejected. A failed trial costs two disruptions (apply, then revert), which is why it is opt-in. Only valid with `rollout: coordinated` -- `independent` has no cohort-wide commit to confirm and nothing that could revert one. |
