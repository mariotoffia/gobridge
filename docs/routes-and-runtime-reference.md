...
| `replay_budget` | duration | no | `15m` | **`shared_outbox` only:** wall-clock ceiling, measured from a record's FIRST attempt, on how long the outbox drainer keeps redelivering it. A `direct_hold` route never reads it -- its in-process bound is `send_retry_budget`. It is the **age** half of the poison gate and applies **AND**-ed with `max_replay_attempts`: a record is poisoned to the DLQ only once **both** the attempt count and this budget are spent, so raising one alone changes nothing. Negative is rejected at config load; `0` takes the default. |
| `send_retry_budget` | duration | no | `60s` | **`direct_hold` only:** how long a recoverable send failure is retried inside the bridge, with the route's backoff and the source message still held, before the message is handed back to its source or dead-lettered. The outbox drainer never reads it, just as a `direct_hold` route never reads `replay_budget`. **Omitting the field takes the `60s` default; an explicit `0s` turns in-process retry off** and restores the one-send-per-delivery behaviour of releases before in-process send retry existed. Negative is rejected at config load. Two further checks run at config load: the budget is counted into the worst-case pipeline time compared against a FIXED source visibility window (skipped when the source auto-extends), and `send_retry_budget` + the send wedge ceiling must fit inside the source session's settlement-recovery wait. Both checks size the last send by that ceiling, `send_timeout` + `min(send_timeout, 5s)`, the longest a send that ignores its timeout holds the message; the rejections print it as the send timeout followed by `(+5s wedge grace)`. **The route's `backoff` now decides how many times a failing destination is contacted inside the budget** -- in-process retries are bounded by wall clock, not by `max_replay_attempts` as the pre-retry behaviour was, and no wait between two sends is shorter than 100 ms: a shorter backoff interval or `RetryAfter` hint is raised to it. With the default ladder (1s doubling to 30s) a 60s budget is about six sends; a deliberately aggressive ladder (`initial_interval: 1ms` with `multiplier: 1`, both legal) is raised to the 100 ms floor, which still allows about 600 sends (budget ÷ 100 ms) at a destination that is already unwell. Keep the default backoff, or lower the budget. Counted by `SendRetries` and `SendRetryBudgetExhausted`. See [ADR 0017](adr/0017-direct-hold-in-process-send-retry.md) |
