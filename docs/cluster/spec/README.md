...
| **Raft membership** (Ongaro §4) | One change at a time with quorum overlap; abort the change if a member can't come along; fall back to the prior config entry. | Freeze-and-abort on membership change (F6); no member applies an unagreed generation (G2). |
