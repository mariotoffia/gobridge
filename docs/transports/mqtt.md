...
| any | either | any | any | permanent failure or expiry reached | Terminal per `OnPermanentFailure` / `OnExpired` (default `dlq`): `dlq` writes the record/envelope to the DLQ, counts `MetricDLQEntries`, then ACKs. A DLQ **write failure** does not drop — the delivery/record is retried (source or outbox redelivers). `drop` (or no DLQ store configured) records a drop metric, logs, and ACKs — loss by design. |
