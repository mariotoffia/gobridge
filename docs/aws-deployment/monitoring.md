...
| `RouteErrors` | `route_id` | Count | Delivery errors by route: a recoverable send failure, a recoverable processor-chain failure, and a resolver that returned more than one plan for a single-dispatch route. For the **send** half on a `direct_hold` route, it is raised when the in-process send retry gives up, not on the first failed send, so a stalled destination surfaces here up to `send_retry_budget` plus the last send's hold late -- that hold is at most one send wedge ceiling, `send_timeout` + `min(send_timeout, 5s)`, so 95s with the defaults (60s + 35s) -- or never, if it recovers inside the budget. Alert on `SendRetries` for the early signal. The other two are unaffected. |
