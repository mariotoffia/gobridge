...
| `RouteErrors` | `route_id` | Count | Delivery errors by route: a recoverable send failure, a recoverable processor-chain failure, and a resolver that returned more than one plan for a single-dispatch route. For the **send** half on a `direct_hold` route, it is raised when the in-process send retry gives up, not on the first failed send, so under the default `send_retry_budget` (60s) a stalled destination surfaces here up to a minute late -- or never, if it recovers inside the budget. Alert on `SendRetries` for the early signal. The other two are unaffected. |
