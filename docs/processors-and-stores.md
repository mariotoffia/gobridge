...
are supplied when you construct it in Go (see [Programmatic Registration](store-backends.md#programmatic-registration));
...
[deployment guide](deployment-scaling.md#shared-tenant-usage-store).
...
Store backends — the memory, SQLite and DynamoDB options for each store role, and programmatic store registration — are documented on their own page: [Store backends](store-backends.md).
