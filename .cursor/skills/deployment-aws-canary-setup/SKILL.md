...
Integrate canary deployment into CI/CD pipelines.

```yaml
# .github/workflows/deploy-prod-canary.yaml
  ...
  deploy-canary:
    ...
    steps:
      ...
      - name: Wait and check metrics
        run: |
          sleep 300
          ./scripts/check-metrics.sh ${{ inputs.service }} prod

      - name: Rollback on failure
        if: failure()
        run: ./scripts/rollback.sh ${{ inputs.service }} prod
```
...
- [ ] GitHub Actions workflow includes approval gates for production
