...
compatibility: "Schema and Contract Strategy, Event-Driven Architecture, API Design"
metadata:
  type: reviewer
  patterns:
...
|-------------|---------|--------------|
| Add required field | `"facility_id": { "type": "string" }` now required | Existing requests fail validation |
...
|-------------|---------|----------|
| Add optional field | `"nickname": { "type": "string" }` (optional) | Old consumers ignore it |
...
- Event type format: `{context}.{aggregate}.{action}` (e.g., `asset.battery.registered`)
- Required fields: `event_id`, `event_type`, `schema_version`, `occurred_at`, `correlation_id`
- Producer context in event type prefix (`asset.`, `facility.`, `grid.`)

### API Versioning
- URL path versioning: `/v1/`, `/v2/`
- All breaking changes require new version path
- Deprecation header on sunset endpoints: `Sunset: Sat, 31 Dec 2025 23:59:59 GMT`

### Schema Registry
- All production schemas registered in central registry
- Schema compatibility checked before deployment
- Consumer schemas validated against registry
