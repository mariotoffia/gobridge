...
context: observability, logging, metrics, tracing, monitoring, alerting
---
...
When working with observability, you can use:
- **cloudwatch-mcp-server**: Query logs, create dashboards, manage alarms
...
METHOD ObservabilityMiddleware.WrapMessageHandler(handler: MessageHandlerFunc) RETURNS MessageHandlerFunc
    RETURN FUNCTION(ctx: Context, event: MessageEvent) RETURNS Result<Void, Error>
        FOR EACH record IN event.Records DO
            // Extract correlation ID from message attributes
            correlationId = ""
            IF record.MessageAttributes.Contains("CorrelationID") THEN
            ...
            recordCtx = WithCorrelationID(ctx, correlationId)

            this.logger.Info(recordCtx, "Processing message", {
                "message_id": record.MessageId,
                "queue": record.SourceARN
            })
        END FOR
...
METHOD MetricsCollector.RecordSaturation(ctx: Context, resource: String, current: Float64, capacity: Float64)
    ...
    datum = MetricDatum{
        ...
        dimensions: MergeMaps(this.dimensions, {"Resource": resource}),
        timestamp: CurrentTimeUTC()
    }
    this.buffer.Add(datum)
END METHOD

METHOD MetricsCollector.RecordBusinessMetric(ctx: Context, name: String, value: Float64, unit: MetricUnit)
    datum = MetricDatum{
        name: name,
        value: value,
        unit: unit,
...
FUNCTION GetErrorType(err: Error) RETURNS String
    IF err IS NULL THEN
        RETURN "None"
    END IF
    // Could use type checking to get specific error types
    RETURN "UnknownError"
END FUNCTION
```

Metrics should include latency (p50, p95, p99), error rate, and saturation. Business metrics like "optimizations completed" prove the system delivers value.
...
METHOD Tracer.StartSubsegment(ctx: Context, name: String) RETURNS (Context, SegmentCloser)
    ctx, subsegment = BeginSubsegment(ctx, name)
...
METHOD Tracer.AddMetadata(ctx: Context, key: String, value: Any)
    segment = GetSegment(ctx)
    IF segment IS NOT NULL THEN
...
FUNCTION TraceDataOperation(ctx: Context, operation: String, tableName: String, fn: Function() RETURNS Error) RETURNS Error
    ctx, subsegment = BeginSubsegment(ctx, "Database." + operation)
    DEFER subsegment.Close(NULL)

    subsegment.AddAnnotation("table", tableName)
...
CONSTRUCTOR NewTracedHandler(
    serviceName: String,
    metrics: MetricsCollector,
    handler: APIHandlerFunc
) RETURNS TracedHandler
...
METHOD TracedHandler.Handle(ctx: Context, request: APIRequest) RETURNS Result<APIResponse, Error>
    start = CurrentTime()
    ...
    // Start trace segment
    ctx, closeSegment = this.tracer.StartSegment(ctx, request.Path)
    DEFER closeSegment(NULL)

    // Add trace annotations for searchability
    this.tracer.AddAnnotation(ctx, "correlation_id", correlationId)
    this.tracer.AddAnnotation(ctx, "http_method", request.HTTPMethod)
    ...
    // Execute handler
    result = this.handler(ctx, request)

    // Record metrics
    duration = CurrentTime() - start
    operation = request.HTTPMethod + " " + request.Path

    this.metrics.RecordLatency(ctx, operation, duration)

    IF result.IsError() THEN
        this.metrics.RecordError(ctx, operation, "ServerError")
    ELSE
        response = result.Value()
        IF response.StatusCode >= 500 THEN
            this.metrics.RecordError(ctx, operation, "ServerError")
        ELSE IF response.StatusCode >= 400 THEN
            this.metrics.RecordError(ctx, operation, "ClientError")
        ELSE
            this.metrics.RecordSuccess(ctx, operation)
        END IF
    END IF

    // Flush metrics
    this.metrics.Flush(ctx)
    ...
    this.logger.Info(ctx, "Request completed", {
        "method": request.HTTPMethod,
        "path": request.Path,
        "status_code": result.IsOk() ? result.Value().StatusCode : 500,
        "duration_ms": duration.Milliseconds()
    })
...
METHOD DLQMonitor.checkQueue(ctx: Context, name: String, url: String) RETURNS Result<Void, Error>
    // Get queue attributes
    result = this.queueClient.GetQueueAttributes(ctx, url, [
    ...
    IF result.IsError() THEN
        RETURN Error("failed to get queue attributes: " + result.Error().Message())
    END IF

    attributes = result.Value()

    // Parse attributes
    visible = ParseInt(attributes["ApproximateNumberOfMessages"])
    notVisible = ParseInt(attributes["ApproximateNumberOfMessagesNotVisible"])
    ageSeconds = ParseInt(attributes["ApproximateAgeOfOldestMessage"])

    totalDepth = visible + notVisible

    // Log if queue has messages
    IF totalDepth > 0 THEN
        this.logger.Warn(ctx, "DLQ contains messages", {
            "queue_name": name,
            "depth": totalDepth,
    ...
    // Record metrics
    this.metrics.RecordBusinessMetric(ctx, "DLQDepth", Float64(totalDepth), MetricUnit.COUNT)
    this.metrics.RecordBusinessMetric(ctx, "DLQAgeSeconds", Float64(ageSeconds), MetricUnit.COUNT)
...
TYPE DLQAlertConfig
    queueName: String
    depthThreshold: Int64       // Alert when depth exceeds this
    ageThreshold: Duration      // Alert when oldest message exceeds this age
...
METHOD DashboardBuilder.AddLatencyWidget(title: String, operations: List<String>, x: Int, y: Int, width: Int, height: Int)
    ...
    FOR EACH op IN operations DO
        ...
        metrics.Add([this.namespace, "Latency", "Service", this.serviceName, "Operation", op, {"stat": "p50", "label": op + " p50"}])
        metrics.Add(["...", {"stat": "p95", "label": op + " p95"}])
        metrics.Add(["...", {"stat": "p99", "label": op + " p99"}])
    END FOR
...
METHOD DashboardBuilder.AddErrorRateWidget(title: String, x: Int, y: Int, width: Int, height: Int)
    widget = {
        "type": "metric",
        "x": x,
        "y": y,
        ...
        "properties": {
            ...
            "view": "timeSeries",
            "metrics": [
                [this.namespace, "Errors", "Service", this.serviceName, {"stat": "Sum", "label": "Errors"}],
                [this.namespace, "Success", "Service", this.serviceName, {"stat": "Sum", "label": "Success"}]
            ],
            "period": 60
        }
    }
...
METHOD DashboardBuilder.AddDLQWidget(title: String, queueNames: List<String>, x: Int, y: Int, width: Int, height: Int)
    metrics = NewList()
    FOR EACH q IN queueNames DO
        metrics.Add(["AWS/SQS", "ApproximateNumberOfMessagesVisible", "QueueName", q, {"label": q}])
    END FOR
...
METHOD DashboardBuilder.Build(ctx: Context) RETURNS Result<Void, Error>
    ...
    result = this.publisher.PutDashboard(ctx, dashboardName, bodyJson)
    IF result.IsError() THEN
        RETURN Error("failed to create dashboard: " + result.Error().Message())
    END IF
...
Dashboards should show: latency percentiles, error rates, throughput, DLQ depth, and key business metrics.
...
TYPE AlarmConfig
    ...
    owner: String  // For documentation
...
INTERFACE AlarmPublisher
    METHOD CreateAlarm(ctx: Context, config: AlarmConfig) RETURNS Result<Void, Error>
END INTERFACE

TYPE AlarmManager
    publisher: AlarmPublisher

CONSTRUCTOR NewAlarmManager(publisher: AlarmPublisher) RETURNS AlarmManager
    RETURN AlarmManager{publisher: publisher}
END CONSTRUCTOR

METHOD AlarmManager.CreateAlarm(ctx: Context, config: AlarmConfig) RETURNS Result<Void, Error>
    RETURN this.publisher.CreateAlarm(ctx, config)
END METHOD
...
METHOD AlarmManager.CreateStandardAlarms(ctx: Context, serviceName: String, namespace: String, notificationTarget: String) RETURNS Result<Void, Error>
    serviceDimension = {"Service": serviceName}
...
GLOBAL handler: TracedHandler

FUNCTION Initialize()
    ctx = BackgroundContext()
    serviceName = GetEnvironmentVariable("SERVICE_NAME")
    metricsNamespace = GetEnvironmentVariable("METRICS_NAMESPACE")

    // Create observability components
    metricsPublisher = NewCloudMetricsPublisher()
    queueClient = NewQueueClient()

    metrics = NewMetricsCollector(metricsPublisher, metricsNamespace, serviceName)
    logger = NewLogger(serviceName)

    // Set up DLQ monitoring
    dlqMonitor = NewDLQMonitor(queueClient, logger, metrics)
    dlqMonitor.RegisterQueue("events-dlq", GetEnvironmentVariable("DLQ_URL"))

    // Create traced handler
    handler = NewTracedHandler(
        serviceName,
        metrics,
        businessHandler  // Your actual handler
    )
END FUNCTION

FUNCTION Main()
    StartHandler(handler.Handle)
END FUNCTION
```
