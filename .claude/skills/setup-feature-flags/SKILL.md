...
context: feature-flags, deployment, release-management, rollout, kill-switch
---
...
```pseudocode
TYPE ConfigClient IMPLEMENTS FeatureFlagClient
    configService: ConfigurationService
    configToken: String
    cache: Map<FlagKey, Boolean>
    cacheMutex: Mutex
    registry: FlagRegistry
    stopChannel: Channel

CONSTRUCTOR NewConfigClient(config: CloudConfig, appId: String, envId: String, profileId: String, registry: FlagRegistry) RETURNS Result<ConfigClient, Error>
    service = NewConfigurationService(config)

    sessionResult = service.StartSession({
        applicationId: appId,
        environmentId: envId,
        profileId: profileId
    })

    IF sessionResult.IsError() THEN
        RETURN sessionResult.Error()
    END IF

    client = ConfigClient{
        configService: service,
        configToken: sessionResult.Value().InitialToken,
        cache: NewMap(),
        registry: registry,
        stopChannel: NewChannel()
    }

    refreshResult = client.refresh(BackgroundContext())
    IF refreshResult.IsError() THEN
        RETURN refreshResult.Error()
    END IF

    StartGoroutine(client.poll)
    RETURN Ok(client)
END CONSTRUCTOR

METHOD ConfigClient.IsEnabled(ctx: Context, key: FlagKey, evalCtx: EvaluationContext) RETURNS Boolean
    this.cacheMutex.RLock()
    enabled, exists = this.cache[key]
    ...
    IF NOT exists THEN
        ...
        IF config.state == FlagState.ROLLOUT THEN
            RETURN this.isInRollout(evalCtx.userId, config.rolloutPercentage)
        END IF
...
METHOD ConfigClient.isInRollout(userId: String, percentage: Int) RETURNS Boolean
    ...
    IF percentage <= 0 THEN
        RETURN FALSE
    END IF
    hash = FNVHash32(userId)
    RETURN (hash MOD 100) < percentage
END METHOD

METHOD ConfigClient.refresh(ctx: Context) RETURNS Result<Void, Error>
    output = this.configService.GetLatestConfiguration({
    ...
    this.configToken = output.Value().NextPollToken

    IF output.Value().Configuration.Length() > 0 THEN
        flagValues = JsonDeserialize<Map<String, Boolean>>(output.Value().Configuration)
        this.cacheMutex.Lock()
        FOR EACH (key, value) IN flagValues DO
            this.cache[FlagKey(key)] = value
        END FOR
...
METHOD ConfigClient.poll()
    ticker = NewTicker(30 * Second)
    DEFER ticker.Stop()

    LOOP
        SELECT
            CASE <-ticker.Channel():
                this.refresh(BackgroundContext())
...
METHOD ConfigClient.Close() RETURNS Result<Void, Error>
...
```makefile
check-flags:
	go run cmd/cleanup/main.go || (echo "ERROR: Expired flags" && exit 1)
```
