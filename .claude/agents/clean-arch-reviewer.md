...
**Code under review:**
```pseudocode
// domain/building.py or domain/building.go
...
#### Warnings (Should Fix)
None - the critical issues must be addressed first.

#### Suggestions (Improvements)
1. Add validation in constructor
2. Raise domain events on state changes
...
### Positive Patterns Observed
- `OptimizeHeating()` contains pure business logic - keep this pattern
- Domain type naming follows ubiquitous language

### Recommended Refactoring

1. **Define port in domain:**
```pseudocode
// domain/building_repository.py or domain/building.go
...
### 4. Shared Utils Package
```pseudocode
// BAD: Everything imports utils
IMPORT utils
```
**Impact**: Hidden coupling, unclear ownership.
**Fix**: Scope utilities to layers, use domain-specific helpers.

### 5. Transport Types in Ports
```pseudocode
// BAD: domain/ports.py
...
### 6. Infrastructure Struct Embedding
```pseudocode
// BAD: Embedding database types
TYPE Asset
    EMBED DatabaseRecord
    id: String
END TYPE
```
**Impact**: Domain polluted with persistence concerns.
**Fix**: Map in adapter, keep domain types pure.

## -Specific Checks

### Entities Layer (innermost)
Expected: `EnergyAsset`, `HeatingBuilding`, `HeatCurve`, `PowerPlant`, `MarketStrategy`
- Pure structs with behavior
- No serialization tags (no JSON, no database)
- Domain events raised from aggregate methods

### Use Cases Layer
Expected: `OptimizeBuildingHeating`, `RegisterNewAsset`, `CalculateOptimalSchedule`
- Accept repository/provider interfaces
- Orchestrate domain operations
- No direct SDK calls

### Interface Adapters Layer
Expected: `DatabaseBuildingRepository`, `WeatherProviderAdapter`, `HeatingScheduleController`
- Implement domain interfaces
- Handle mapping and serialization
- Contain retry/backoff logic

### Infrastructure Layer
Expected: Lambda handlers, `main` wiring, event bindings
- Wire dependencies together
- Configure SDK clients
- Set up HTTP/Lambda entry points
