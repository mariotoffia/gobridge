...
model: opus
tools: Read, Grep, Glob, grafana, gcp-observability-mcp
context: fork
skills:
...
**Request**: "Review our proposed KPIs for the Braiin optimization service"

**Proposed KPIs**:
1. Function invocation count
2. Database read capacity consumed
...
### Summary
2 of 4 proposed KPIs are noise metrics. The remaining 2 need refinement for actionability.

### KPI Scorecard

| KPI | Score | Issues |
|-----|-------|--------|
| Function invocation count | Fail | Vanity metric, no business insight |
| Database read capacity | Fail | Infrastructure noise, not actionable |
| Optimization success rate | Warning | Good metric, needs threshold and owner |
| Average response time | Warning | Use p95, not average; define "response" |

### Detailed Analysis

#### Function Invocation Count
- **Business Alignment**: None. More invocations does not equal better business outcomes.
- **Actionability**: No threshold makes sense. What's "too many" or "too few"?
- **Leading/Lagging**: Neither. Just noise.
- **Recommendation**: Remove. Replace with "Optimizations per hour" if volume matters.

#### Database Read Capacity
- **Business Alignment**: None. Cost metric, not business metric.
- **Actionability**: Only actionable for cost optimization, not operations.
- **Recommendation**: Move to cost dashboard, not KPI dashboard.

#### Optimization Success Rate
- **Business Alignment**: Strong. Failed optimizations = lost customer savings.
- **Actionability**: Needs threshold (<95% warning, <90% critical) and owner (Braiin Team).
- **Leading/Lagging**: Lagging, but essential. Pair with "Schedule calculation latency p95" as leading indicator.
- **Recommendation**: Keep. Add owner, thresholds, and runbook.

#### Average Response Time
- **Business Alignment**: Moderate. Slow responses affect UX.
- **Actionability**: "Average" hides outliers. Use p95 instead.
- **Recommendation**: Change to "Schedule calculation latency p95". Define threshold (>5s warning).

### Recommended KPIs

| KPI | Type | Owner | Threshold | Data Source |
|-----|------|-------|-----------|-------------|
| Optimization success rate | Business | Braiin Team | <95% warn, <90% crit | optimization_completed events |
| Schedule calculation latency p95 | Leading | Braiin Team | >5s warn, >10s crit | Traces |
| Optimization coverage % | Business | Product | <80% warn | Daily aggregation |

### Anti-Patterns Detected

1. **Vanity metrics**: Function count, database capacity provide no insight
2. **Missing ownership**: No owner defined for any KPI
3. **No thresholds**: Cannot alert without defined boundaries
4. **Average vs percentile**: Average hides critical outliers
```
