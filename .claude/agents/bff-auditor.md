...
| Violation | Why It's Wrong | Impact |
|-----------|----------------|--------|
| Contain domain logic | Duplicates rules, creates divergence | Inconsistent behavior |
| Own data | Distributed consistency problem | State conflicts |
| Make domain decisions | Violates BC autonomy | Deployment coupling |
| Access databases directly | Bypasses domain validation | Data corruption |
| Emit domain events | BFF is view layer only | Ownership confusion |
...
## Common BFF Anti-Patterns to Flag

| Anti-Pattern | Example | Fix |
|--------------|---------|-----|
| Domain logic in BFF | `if asset.StateOfCharge < 20` | Move to BC, expose via API |
